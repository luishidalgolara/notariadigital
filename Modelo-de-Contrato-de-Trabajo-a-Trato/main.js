// ========================================
// 📋 SISTEMA PRINCIPAL DE CONTRATO DE TRABAJO A TRATO
// ========================================
...
const systemState = {
    formFields: [
        ...
        'nombreEmpresa', 'rutEmpresa', 'ciudadEmpresa', 'direccionEstablecimiento',
        'nombreRepresentante', 'rutRepresentante', 'nacionalidadRepresentante',
        'nombreTrabajador', 'rutTrabajador', 'fechaNacimientoTrabajador', 'ciudadTrabajador', 'direccionTrabajador', 'fechaIngreso',
        'descripcionPiezasA', 'descripcionPiezasB', 'cantidadMinimaSemanal', 'precioPorPieza', 'frecuenciaPago', 'duracionContrato',
        'nombreEstablecimiento', 'direccionTrabajo', 'diaInicioJornada', 'diaTerminoJornada', 'horaInicio', 'horaTermino',
        'beneficioA', 'beneficioB', 'beneficioC',
        'pactosAdicionales', 'prohibicionesRestricciones', 'observacionesGenerales'
    ],
...
class ContratoTratoSystem {
    ...
    async init() {
        console.log('📋 Inicializando Sistema de Contrato de Trabajo a Trato...');
    ...
    setupSpecialFieldListeners() {
        ...
        const precioPorPieza = document.getElementById('precioPorPieza');
        if (precioPorPieza) {
            precioPorPieza.addEventListener('input', (e) => {
                const precio = parseFloat(e.target.value);
                if (precio && !ConfigUtils.validatePrecioPieza(precio)) {
                    this.showNotification('El precio por pieza debe estar entre $100 y $50.000', 'warning');
                }
                this.updatePreview();
            });
        }

        // Validar cantidad mínima semanal
        const cantidadMinimaSemanal = document.getElementById('cantidadMinimaSemanal');
        if (cantidadMinimaSemanal) {
            cantidadMinimaSemanal.addEventListener('input', (e) => {
                const cantidad = parseInt(e.target.value);
                if (cantidad && !ConfigUtils.validateCantidadMinima(cantidad)) {
                    this.showNotification('La cantidad debe estar entre 1 y 10.000 unidades', 'warning');
                }
                this.updatePreview();
            });
        }
        ...
        // Validar fecha de nacimiento (calcular edad automáticamente si se desea)
        const fechaNacimiento = document.getElementById('fechaNacimientoTrabajador');
        if (fechaNacimiento) {
            fechaNacimiento.addEventListener('change', (e) => {
                const edad = ConfigUtils.calculateAge(e.target.value);
                if (edad < 18) {
                    this.showNotification('El trabajador debe ser mayor de edad (18 años)', 'warning');
                } else if (edad > 100) {
    ...
    updateDuplicateReferences() {
        ...
        if (prev_ciudadContrato2) prev_ciudadContrato2.textContent = ciudadContrato || '________';
    }
    ...
    updateDatePreview() {
        ...
        // Actualizar fecha de nacimiento formateada
        ...
        if (prev_fechaNacimiento) {
            if (fechaNacimiento && ConfigUtils.validateDate(fechaNacimiento)) {
                prev_fechaNacimiento.textContent = ConfigUtils.formatDateToSpanish(fechaNacimiento);
            } else {
                prev_fechaNacimiento.textContent = '__________';
            }
        }

        // Actualizar fecha de ingreso formateada
        const fechaIngreso = document.getElementById('fechaIngreso').value;
        const prev_fechaIngreso = document.getElementById('prev_fechaIngreso');
        if (prev_fechaIngreso) {
            if (fechaIngreso && ConfigUtils.validateDate(fechaIngreso)) {
                prev_fechaIngreso.textContent = ConfigUtils.formatDateToSpanish(fechaIngreso);
            } else {
                prev_fechaIngreso.textContent = '__________';
            }
        }
    }
    ...
    updateSpecialFields() {
        // Formatear precio por pieza con moneda
        const precioPorPieza = document.getElementById('precioPorPieza').value;
        const prev_precioPorPieza = document.getElementById('prev_precioPorPieza');
        if (prev_precioPorPieza) {
            if (precioPorPieza && !isNaN(precioPorPieza)) {
                prev_precioPorPieza.textContent = ConfigUtils.formatNumber(precioPorPieza);
            } else {
                prev_precioPorPieza.textContent = '_______';
            }
        }

        // Capitalizar días de la semana
        const diaInicioJornada = document.getElementById('diaInicioJornada').value;
        const diaTerminoJornada = document.getElementById('diaTerminoJornada').value;
        
        const prev_diaInicioJornada = document.getElementById('prev_diaInicioJornada');
        const prev_diaTerminoJornada = document.getElementById('prev_diaTerminoJornada');
        
        if (prev_diaInicioJornada) {
            prev_diaInicioJornada.textContent = diaInicioJornada ? ConfigUtils.capitalize(diaInicioJornada) : '________';
        }
        if (prev_diaTerminoJornada) {
            prev_diaTerminoJornada.textContent = diaTerminoJornada ? ConfigUtils.capitalize(diaTerminoJornada) : '________';
        }
    }
    ...
    getPlaceholderText(field) {
        const placeholders = {
            ...
            'ciudadEmpresa': '________',
            'direccionEstablecimiento': '________________',
            ...
            'ciudadTrabajador': '________',
            'direccionTrabajador': '________________',
            'descripcionPiezasA': '________________________________',
            'descripcionPiezasB': '________________________________',
            'cantidadMinimaSemanal': '___',
            'precioPorPieza': '_______',
            'frecuenciaPago': '________________',
            'duracionContrato': '________________',
            'nombreEstablecimiento': '________________',
            'direccionTrabajo': '________________',
            'diaInicioJornada': '________',
            'diaTerminoJornada': '________',
            'horaInicio': '_____',
            'horaTermino': '_____',
            'beneficioA': '________________',
            'beneficioB': '________________',
            'beneficioC': '________________',
            'pactosAdicionales': '________________',
            'prohibicionesRestricciones': '________________',
    ...
    clearForm() {
        this.showConfirmDialog(
            ...
            () => {
                ...
                const diaInicioJornada = document.getElementById('diaInicioJornada');
                if (diaInicioJornada) diaInicioJornada.value = 'lunes';
                
                const diaTerminoJornada = document.getElementById('diaTerminoJornada');
                if (diaTerminoJornada) diaTerminoJornada.value = 'viernes';
                
                const horaInicio = document.getElementById('horaInicio');
                if (horaInicio) horaInicio.value = '08:00';
                
                const horaTermino = document.getElementById('horaTermino');
                if (horaTermino) horaTermino.value = '17:00';
                
                const frecuenciaPago = document.getElementById('frecuenciaPago');
                if (frecuenciaPago) frecuenciaPago.value = 'semanal';
    ...
    async generatePDF() {
        ...
        try {
            ...
            setTimeout(() => {
                doc.save(fileName);
                this.showNotification('PDF de Contrato de Trabajo a Trato generado exitosamente', 'success');
            }, 1500);
    ...
    async addPDFContent(doc) {
        ...
        doc.setFontSize(18);
        doc.text('CONTRATO DE TRABAJO A TRATO', 105, y, { align: 'center' });
        y += 30;
        ...
        const nombreRepresentante = document.getElementById('nombreRepresentante').value;
        const rutRepresentante = document.getElementById('rutRepresentante').value;
        const direccionEstablecimiento = document.getElementById('direccionEstablecimiento').value;
        
        const nombreTrabajador = document.getElementById('nombreTrabajador').value;
        const rutTrabajador = document.getElementById('rutTrabajador').value;
        const direccionTrabajador = document.getElementById('direccionTrabajador').value;
        const ciudadTrabajador = document.getElementById('ciudadTrabajador').value;
        const fechaNacimientoTrabajador = document.getElementById('fechaNacimientoTrabajador').value;

        // Párrafo de fecha y partes
        let fechaTexto = '__ días del mes de ________ de 20__';
        if (fechaContrato && ConfigUtils.validateDate(fechaContrato)) {
            const date = new Date(fechaContrato);
            fechaTexto = `${date.getDate()} días del mes de ${this.mesesEspanol[date.getMonth()]} de ${date.getFullYear()}`;
        }

        let fechaNacimientoTexto = '__________';
        if (fechaNacimientoTrabajador && ConfigUtils.validateDate(fechaNacimientoTrabajador)) {
            fechaNacimientoTexto = ConfigUtils.formatDateToSpanish(fechaNacimientoTrabajador);
        }

        const parrafo1 = `En ${ciudadContrato}, a ${fechaTexto}, entre la Empresa ${nombreEmpresa}, representada legalmente por ${nombreRepresentante}, RUT N° ${rutRepresentante}, ambos domiciliados para estos efectos en el lugar en que funciona el Establecimiento, calle ${direccionEstablecimiento}, de esta misma ciudad, en adelante el EMPLEADOR y el señor ${nombreTrabajador}, RUT N° ${rutTrabajador}, con domicilio en la calle ${direccionTrabajador} de la ciudad de ${ciudadTrabajador}, fecha de nacimiento ${fechaNacimientoTexto}, en adelante el TRABAJADOR, se celebra el presente Contrato de Trabajo a Trato:`;
        
        doc.text(parrafo1, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, parrafo1, pageWidth - 2 * margin) + 20;

        // Descripción de las piezas o unidades
        const descripcionPiezasA = document.getElementById('descripcionPiezasA').value;
        const descripcionPiezasB = document.getElementById('descripcionPiezasB').value;

        const seccionPiezas = `El trabajador deberá confeccionar o elaborar las siguientes piezas o unidades:\n\na.- ${descripcionPiezasA}\n\nb.- ${descripcionPiezasB}\n\n(Indicar detalladamente la naturaleza de los servicios).`;
        
        doc.text(seccionPiezas, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, seccionPiezas, pageWidth - 2 * margin) + 15;

        // Cantidad mínima semanal
        const cantidadMinimaSemanal = document.getElementById('cantidadMinimaSemanal').value;
        
        const clausulaCantidad = `El trabajador tiene como obligación rendir semanalmente la cantidad de ${cantidadMinimaSemanal} unidades, como mínimo;`;
        
        doc.text(clausulaCantidad, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausulaCantidad, pageWidth - 2 * margin) + 15;
        ...
        const clausulaLugar = `El trabajador prestará sus servicios en el Establecimiento denominado ${nombreEstablecimiento}, ubicado en la calle ${direccionTrabajo} de la ciudad de ${ciudadContrato}.`;
        
        doc.text(clausulaLugar, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausulaLugar, pageWidth - 2 * margin) + 15;
        ...
        const clausulaJornada = `La duración y distribución de la jornada de trabajo será: ${ConfigUtils.capitalize(diaInicioJornada)} a ${ConfigUtils.capitalize(diaTerminoJornada)}, de ${horaInicio} horas hasta las ${horaTermino} horas.`;
        
        doc.text(clausulaJornada, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausulaJornada, pageWidth - 2 * margin) + 15;
        ...
        const clausulaPago = `Por cada pieza fabricada (o trabajo específico ejecutado) recibirá la cantidad de $${ConfigUtils.formatNumber(precioPorPieza)} pagada en el período que se indica ${frecuenciaPago}.`;
        
        doc.text(clausulaPago, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausulaPago, pageWidth - 2 * margin) + 15;
        ...
        const clausulaBeneficios = `El empleador se compromete a otorgar o suministrar al trabajador los siguientes beneficios:\na) ${beneficioA}\nb) ${beneficioB}\nc) ${beneficioC}`;
        
        doc.text(clausulaBeneficios, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausulaBeneficios, pageWidth - 2 * margin) + 15;

        // Obligaciones del trabajador
        const clausulaObligaciones = `El trabajador se compromete y obliga expresamente a cumplir las instrucciones que le sean impartidas por su jefe inmediato o por la gerencia de la empresa, en relación a su trabajo, y acatar en todas sus partes las normas del Reglamento Interno de Orden, Higiene y Seguridad (cuando exista en la empresa), las que declara conocer y que forman parte integrante del presente contrato, reglamento del cual se le entrega un ejemplar.`;
        
        doc.text(clausulaObligaciones, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausulaObligaciones, pageWidth - 2 * margin) + 15;

        // Duración del contrato
        const duracionContrato = document.getElementById('duracionContrato').value;

        const clausulaDuracion = `El presente contrato de trabajo tendrá una duración de ${duracionContrato}.`;
        
        doc.text(clausulaDuracion, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausulaDuracion, pageWidth - 2 * margin) + 15;

        // Pactos adicionales
        const pactosAdicionales = document.getElementById('pactosAdicionales').value;
        if (pactosAdicionales.trim()) {
            const clausulaPactos = `Las partes acuerdan los siguientes Pactos:\n\n${pactosAdicionales}`;
            
            doc.text(clausulaPactos, margin, y, {maxWidth: pageWidth - 2 * margin});
            y += this.calculateTextHeight(doc, clausulaPactos, pageWidth - 2 * margin) + 15;
        }

        // Prohibiciones
        const prohibicionesRestricciones = document.getElementById('prohibicionesRestricciones').value;
        if (prohibicionesRestricciones.trim()) {
            const clausulaProhibiciones = `Prohibiciones: ${prohibicionesRestricciones}`;
            
            doc.text(clausulaProhibiciones, margin, y, {maxWidth: pageWidth - 2 * margin});
            y += this.calculateTextHeight(doc, clausulaProhibiciones, pageWidth - 2 * margin) + 15;
        }

        // Fecha de ingreso
        const fechaIngreso = document.getElementById('fechaIngreso').value;
        let fechaIngresoTexto = '__________';
        if (fechaIngreso && ConfigUtils.validateDate(fechaIngreso)) {
            fechaIngresoTexto = ConfigUtils.formatDateToSpanish(fechaIngreso);
        }

        const clausulaIngreso = `Se deja expresa constancia que el trabajador ingresó a prestar servicios el ${fechaIngresoTexto}.`;
        
        doc.text(clausulaIngreso, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausulaIngreso, pageWidth - 2 * margin) + 15;

        // Conformidad
        const clausulaFinal = `Para constancia y señal de conformidad firman las partes, quedando el original en poder del EMPLEADOR y la copia para el TRABAJADOR.`;
        
        doc.text(clausulaFinal, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausulaFinal, pageWidth - 2 * margin) + 30;
    ...
    generateFileName() {
        ...
        return `Contrato_Trato_${nombreTrabajador}_${nombreEmpresa}_${timestamp}.pdf`;
    }
    ...
    validateFormWithSignatures() {
        ...
        const requiredFields = [
            ...
            'nombreTrabajador', 'rutTrabajador', 'fechaNacimientoTrabajador', 'ciudadTrabajador', 'direccionTrabajador',
            'descripcionPiezasA', 'cantidadMinimaSemanal', 'precioPorPieza', 'frecuenciaPago', 'duracionContrato',
            'nombreEstablecimiento', 'direccionTrabajo', 'diaInicioJornada', 'diaTerminoJornada', 'horaInicio', 'horaTermino'
        ];
        ...
        // Validar precio por pieza
        const precioPorPieza = document.getElementById('precioPorPieza').value;
        if (precioPorPieza && !ConfigUtils.validatePrecioPieza(precioPorPieza)) {
            issues.push('Precio por pieza inválido');
        }
        
        // Validar cantidad mínima
        const cantidadMinima = document.getElementById('cantidadMinimaSemanal').value;
        if (cantidadMinima && !ConfigUtils.validateCantidadMinima(cantidadMinima)) {
            issues.push('Cantidad mínima semanal inválida');
        }
...
class FirmaDigitalProcessor {
    ...
    applySignature() {
        if (this.currentType && this.processedImageData) {
            ...
            if (window.contratoTratoSystem) {
                window.contratoTratoSystem.updateProgress();
            }
            
            this.closeModal();
            
            if (window.contratoTratoSystem) {
                window.contratoTratoSystem.showNotification(`Firma digital del ${this.currentType} aplicada correctamente`, 'success');
            }
        }
    }
...
// ===== CLASE PROCESADOR DE FOTOS CARNET (IGUAL QUE ANTES) =====
class PhotoCarnetDualProcessor {
    ...
    handleCameraError(error) {
        ...
        if (window.contratoTratoSystem) {
            window.contratoTratoSystem.showNotification(errorMessage, 'error');
        }
    }
    ...
    capturePhoto() {
        ...
        try {
            ...
            if (window.contratoTratoSystem) {
                window.contratoTratoSystem.showNotification(`Foto del ${this.currentStep} capturada`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al capturar foto:', error);
            if (window.contratoTratoSystem) {
                window.contratoTratoSystem.showNotification('Error al capturar la foto', 'error');
            }
        }
    ...
    nextStep() {
        if (this.currentStep === 'frente' && this.tempImages.frente) {
            ...
            this.showCamera();
            if (window.contratoTratoSystem) {
                window.contratoTratoSystem.showNotification('Ahora capture el reverso del carnet', 'info');
            }
        } else if (this.currentStep === 'reverso' && this.tempImages.reverso) {
            this.currentStep = 'completed';
            this.showSummary();
            if (window.contratoTratoSystem) {
                window.contratoTratoSystem.showNotification('Ambas fotos capturadas', 'success');
            }
        }
    }
    ...
    restartProcess() {
        ...
        this.showCamera();
        if (window.contratoTratoSystem) {
            window.contratoTratoSystem.showNotification('Proceso reiniciado', 'info');
        }
    }
    ...
    async confirmAllPhotos() {
        if (!this.tempImages.frente || !this.tempImages.reverso || !this.currentType) {
            if (window.contratoTratoSystem) {
                window.contratoTratoSystem.showNotification('Faltan fotos por capturar', 'error');
            }
            return;
        }
        ...
        try {
            ...
            if (window.contratoTratoSystem) {
                window.contratoTratoSystem.showNotification(`Carnet completo del ${this.currentType} guardado`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al confirmar fotos:', error);
            if (window.contratoTratoSystem) {
                window.contratoTratoSystem.showNotification('Error al procesar las fotos', 'error');
            }
        }
    }
...
window.clearForm = function() {
    if (window.contratoTratoSystem) {
        window.contratoTratoSystem.clearForm();
    }
};

window.generatePDF = function() {
    if (window.contratoTratoSystem) {
        window.contratoTratoSystem.generatePDF();
    }
};

// ===== INICIALIZACIÓN DEL SISTEMA =====
let contratoTratoSystem;

function initializeContratoTratoSystem() {
    console.log('📋 Iniciando Sistema de Contrato de Trabajo a Trato...');
    
    // Verificar que jsPDF esté disponible
    if (typeof window.jspdf === 'undefined') {
        console.warn('⚠️ jsPDF no está disponible, esperando...');
        setTimeout(initializeContratoTratoSystem, 1000);
        return;
    }
    ...
    try {
        contratoTratoSystem = new ContratoTratoSystem();
        window.contratoTratoSystem = contratoTratoSystem;
        
        console.log('✅ Sistema completamente inicializado');
    } catch (error) {
        console.error('❌ Error crítico en inicialización:', error);
        setTimeout(initializeContratoTratoSystem, 2000);
    }
}

// ===== MÚLTIPLES PUNTOS DE INICIALIZACIÓN =====
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeContratoTratoSystem);
} else if (document.readyState === 'interactive' || document.readyState === 'complete') {
    setTimeout(initializeContratoTratoSystem, 500);
}
...
window.addEventListener('error', (event) => {
    ...
    if (window.contratoTratoSystem) {
        window.contratoTratoSystem.showNotification(
...
// ===== LOG FINAL =====
console.log('📜 Sistema de Contrato de Trabajo a Trato - Script principal cargado correctamente');