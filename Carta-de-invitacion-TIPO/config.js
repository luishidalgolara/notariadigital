// ========================================
// 📋 CONFIGURACIÓN DEL SISTEMA DE CARTA DE INVITACIÓN
// ========================================

const CONFIG = {
    // ===== CONFIGURACIÓN GENERAL =====
    APP: {
        name: 'Notaría Digital - Carta de Invitación',
        version: '1.0.0',
        author: 'Sistema Notarial Digital Chile',
        description: 'Sistema profesional para generar cartas de invitación con firmas digitales'
    },
    ...
    AUTOSAVE: {
        ...
        key: 'carta_invitacion_form_data',
        max_history: 5
    },
    ...
    MESSAGES: {
        ...
        error: {
            ...
            invalid_rut: 'RUN inválido',
            invalid_document: 'Número de documento inválido',
            invalid_expediente: 'Número de expediente inválido',
            duplicate_data: 'No puede haber datos duplicados',
...
const ConfigUtils = {
    ...
    // Validar documento general
    validateDocument(document) {
        if (!document || typeof document !== 'string') return false;
        
        const cleanDocument = document.replace(/\s/g, '').toUpperCase();
        return cleanDocument.length >= CONFIG.VALIDATION.document.min_length &&
               cleanDocument.length <= CONFIG.VALIDATION.document.max_length &&
               CONFIG.VALIDATION.document.pattern.test(cleanDocument);
    },

    // Validar número de expediente
    validateExpediente(expediente) {
        if (!expediente || typeof expediente !== 'string') return false;
        
        const cleanExpediente = expediente.replace(/\s/g, '').toUpperCase();
        return CONFIG.VALIDATION.expediente.pattern.test(cleanExpediente);
    },

    // Validar cédula extranjera
    validateCedulaExtranjera(cedula) {
        if (!cedula || typeof cedula !== 'string') return false;
        
        const cleanCedula = cedula.replace(/[^0-9]/g, '');
        return CONFIG.VALIDATION.cedula_extranjera.pattern.test(cleanCedula);
    },
...
ConfigUtils.log('info', 'Sistema de configuración de Carta de Invitación inicializado', {
