// ========================================
// 📋 SISTEMA PRINCIPAL DE DECLARACIÓN JURADA NO PARENTESCO
// ========================================
...
const systemState = {
    formFields: [
        ...
        'nombreFirmante1', 'rutFirmante1', 'cedulaFirmante1', 'nacionalidadFirmante1', 
        'fechaNacimientoFirmante1', 'domicilioFirmante1',
        'nombreFirmante2', 'rutFirmante2', 'cedulaFirmante2', 'nacionalidadFirmante2',
        'fechaNacimientoFirmante2', 'domicilioFirmante2',
...
class DeclaracionJuradaNoParentescoSystem {
    constructor() {
    ...
    async init() {
        console.log('📋 Inicializando Sistema de Declaración Jurada No Parentesco...');
    ...
    async ensureCriticalElements() {
        const criticalElements = [
            ...
            'firmaFirmante1', 'firmaFirmante2',
            'photoFirmante1', 'photoFirmante2',
    ...
    setupSpecialFieldListeners() {
        // Validación RUT Firmante 1
        const rutFirmante1Field = document.getElementById('rutFirmante1');
        if (rutFirmante1Field) {
            rutFirmante1Field.addEventListener('input', (e) => {
                this.handleRutInput(e, 'rutFirmante1');
            });
        }

        // Validación RUT Firmante 2
        const rutFirmante2Field = document.getElementById('rutFirmante2');
        if (rutFirmante2Field) {
            rutFirmante2Field.addEventListener('input', (e) => {
                this.handleRutInput(e, 'rutFirmante2');
            });
        }

        // Validación Cédula Firmante 1
        const cedulaFirmante1Field = document.getElementById('cedulaFirmante1');
        if (cedulaFirmante1Field) {
            cedulaFirmante1Field.addEventListener('input', (e) => {
                this.handleCedulaInput(e, 'cedulaFirmante1');
            });
        }

        // Validación Cédula Firmante 2
        const cedulaFirmante2Field = document.getElementById('cedulaFirmante2');
        if (cedulaFirmante2Field) {
            cedulaFirmante2Field.addEventListener('input', (e) => {
                this.handleCedulaInput(e, 'cedulaFirmante2');
            });
        }
        ...
        const fechaNacimiento1Field = document.getElementById('fechaNacimientoFirmante1');
        if (fechaNacimiento1Field) {
            fechaNacimiento1Field.addEventListener('blur', (e) => {
                if (e.target.value && !ConfigUtils.validateBirthDate(e.target.value)) {
                    this.showNotification('Fecha de nacimiento inválida (debe ser mayor de 18 años)', 'warning');
                }
            });
        }

        const fechaNacimiento2Field = document.getElementById('fechaNacimientoFirmante2');
        if (fechaNacimiento2Field) {
            fechaNacimiento2Field.addEventListener('blur', (e) => {
                if (e.target.value && !ConfigUtils.validateBirthDate(e.target.value)) {
                    this.showNotification('Fecha de nacimiento inválida (debe ser mayor de 18 años)', 'warning');
                }
            });
        ...
        const observacionesAdicionalesField = document.getElementById('observacionesAdicionales');
        if (observacionesAdicionalesField) {
            observacionesAdicionalesField.addEventListener('input', () => {
    ...
    updateConditionalSections() {
        ...
        const observacionesAdicionalesContainer = document.getElementById('observacionesAdicionalesContainer');
        
        if (observacionesAdicionales && observacionesAdicionalesContainer) {
            if (observacionesAdicionales.value.trim()) {
                observacionesAdicionalesContainer.style.display = 'block';
            } else {
                observacionesAdicionalesContainer.style.display = 'none';
            }
        }
    }
    ...
    updateDatePreview() {
        ...
        const prev_fechaDocumento = document.getElementById('prev_fechaDocumento');
        
        if (prev_fechaDocumento) {
            if (fechaDocumento && ConfigUtils.validateDate(fechaDocumento)) {
                const formattedDate = ConfigUtils.formatDateToSpanish(fechaDocumento);
                prev_fechaDocumento.textContent = formattedDate;
            } else {
                prev_fechaDocumento.textContent = '___________';
            }
        }
    }

    updateSignatureNames() {
        const nombreFirmante1 = document.getElementById('nombreFirmante1').value;
        const prev_nombreFirmante1 = document.getElementById('prev_nombreFirmante1');
        if (prev_nombreFirmante1) prev_nombreFirmante1.textContent = nombreFirmante1 || '_______________';

        const nombreFirmante2 = document.getElementById('nombreFirmante2').value;
        const prev_nombreFirmante2 = document.getElementById('prev_nombreFirmante2');
        if (prev_nombreFirmante2) prev_nombreFirmante2.textContent = nombreFirmante2 || '_______________';

        // Actualizar RUTs en vista previa
        const rutFirmante1 = document.getElementById('rutFirmante1').value;
        const prev_rutFirmante1 = document.getElementById('prev_rutFirmante1');
        if (prev_rutFirmante1) prev_rutFirmante1.textContent = rutFirmante1 || 'XX.XXX.XXX-X';

        const rutFirmante2 = document.getElementById('rutFirmante2').value;
        const prev_rutFirmante2 = document.getElementById('prev_rutFirmante2');
        if (prev_rutFirmante2) prev_rutFirmante2.textContent = rutFirmante2 || 'XX.XXX.XXX-X';
    }

    getPlaceholderText(field) {
        const placeholders = {
            'ciudadDocumento': '_______',
            'nombreFirmante1': '_______________',
            'rutFirmante1': 'XX.XXX.XXX-X',
            'cedulaFirmante1': 'XX.XXX.XXX-X',
            'nacionalidadFirmante1': '_________',
            'domicilioFirmante1': '_________________',
            'nombreFirmante2': '_______________',
            'rutFirmante2': 'XX.XXX.XXX-X',
            'cedulaFirmante2': 'XX.XXX.XXX-X',
            'nacionalidadFirmante2': '_________',
            'domicilioFirmante2': '_________________',
            'observacionesAdicionales': '_________________'
        };
    ...
    updateProgress() {
        ...
        if (systemState.firmasDigitales.firmante1) bonusScore += 0.5;
        if (systemState.firmasDigitales.firmante2) bonusScore += 0.5;
        if (systemState.fotosCarnet.firmante1.completed) bonusScore += 0.25;
        if (systemState.fotosCarnet.firmante2.completed) bonusScore += 0.25;
    ...
    async initializeSignatureSystemSafe() {
        ...
        const requiredElements = ['firmaModalOverlay', 'firmaFirmante1', 'firmaFirmante2'];
    ...
    async setupSignatureButtonsSafe() {
        ...
        const btnFirmaFirmante1 = document.getElementById('firmaFirmante1');
        const btnFirmaFirmante2 = document.getElementById('firmaFirmante2');
        
        if (!btnFirmaFirmante1 || !btnFirmaFirmante2) {
            throw new Error('Botones de firma no encontrados');
        }
        
        btnFirmaFirmante1.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openSignatureModalSafe('firmante1');
        });

        btnFirmaFirmante2.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openSignatureModalSafe('firmante2');
        });
    ...
    openSignatureModalSafe(type) {
        ...
        let nombreField;
        if (type === 'firmante1') {
            nombreField = document.getElementById('nombreFirmante1');
        } else {
            nombreField = document.getElementById('nombreFirmante2');
        }
        
        if (!nombreField || !nombreField.value.trim()) {
            const tipoNombre = type === 'firmante1' ? 'primer firmante' : 'segundo firmante';
    ...
    async initializePhotoSystemSafe() {
        ...
        const requiredElements = ['photoModalOverlay', 'photoFirmante1', 'photoFirmante2'];
        const missingElements = requiredElements.filter(id => !document.getElementById(id));
    ...
    async setupPhotoButtonsSafe() {
        ...
        const btnPhotoFirmante1 = document.getElementById('photoFirmante1');
        const btnPhotoFirmante2 = document.getElementById('photoFirmante2');
        
        if (!btnPhotoFirmante1 || !btnPhotoFirmante2) {
            throw new Error('Botones de foto no encontrados');
        }
        
        btnPhotoFirmante1.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openPhotoModalSafe('firmante1');
        });

        btnPhotoFirmante2.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openPhotoModalSafe('firmante2');
        });
            ...
            if (savedData) {
                ...
                Object.keys(data).forEach(field => {
                    if (field === 'firmasDigitales') {
                        systemState.firmasDigitales = data.firmasDigitales || { firmante1: false, firmante2: false };
    ...
    initializeFormFields() {
        // Auto-llenar fecha actual para el documento
        const fechaDocumento = document.getElementById('fechaDocumento');
        if (fechaDocumento && !fechaDocumento.value) {
            const today = new Date().toISOString().split('T')[0];
            fechaDocumento.value = today;
    ...
    clearForm() {
        this.showConfirmDialog(
            ...
            () => {
                ...
                systemState.firmasDigitales = { firmante1: false, firmante2: false };
    ...
    async generatePDF() {
        ...
        try {
            ...
            setTimeout(() => {
                doc.save(fileName);
                this.showNotification('Declaración Jurada PDF generada exitosamente', 'success');
            }, 1500);
    ...
    async addPDFContent(doc) {
        ...
        doc.setFontSize(18);
        doc.text('DECLARACIÓN JURADA NO PARENTESCO', 105, y, { align: 'center' });
        y += 30;
        ...
        const declaracionTexto = CONFIG.DOCUMENT.declaration_text;
        doc.text(declaracionTexto, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, declaracionTexto, pageWidth - 2 * margin) + 40;
        ...
        // Firmas
        this.addSignatureSectionDeclaracion(doc, y);
        ...
        let fechaTexto = '___________';
        if (fechaDocumento && ConfigUtils.validateDate(fechaDocumento)) {
            fechaTexto = ConfigUtils.formatDateToSpanish(fechaDocumento);
        }
        
        const ubicacionFecha = `${ciudadDocumento || '_______'}, ${fechaTexto}`;
        doc.setFont('times', 'italic');
        doc.setFontSize(12);
        doc.text(ubicacionFecha, 105, y, { align: 'center' });
        y += 30;
        ...
        doc.setFont('times', 'normal');
        doc.setFontSize(10);
        doc.text('Nota Legal: Esta declaración jurada tiene validez legal según la normativa chilena vigente.', margin, y);
        y += 6;
        doc.text('Importante: La falsedad de esta declaración puede acarrear sanciones legales conforme al Código Penal.', margin, y);
        
        this.addDigitalWatermark(doc);
    }

    addSignatureSectionDeclaracion(doc, y) {
        ...
        doc.setFont('times', 'bold');
        doc.setFontSize(11);
        doc.text(nombreFirmante1, leftX, y, { align: 'center' });
        doc.text(nombreFirmante2, rightX, y, { align: 'center' });
        y += 8;
        ...
        doc.setFont('times', 'normal');
        doc.setFontSize(9);
        doc.text(`RUT: ${rutFirmante1}`, leftX, y, { align: 'center' });
        doc.text(`RUT: ${rutFirmante2}`, rightX, y, { align: 'center' });
        y += 8;

        // Etiquetas de firmantes
        doc.setFont('times', 'normal');
        doc.setFontSize(10);
        doc.text('Firmante 1', leftX, y, { align: 'center' });
        doc.text('Firmante 2', rightX, y, { align: 'center' });
        
        // Agregar firmas digitales si existen
        if (window.firmaProcessor) {
            try {
                const firmaFirmante1 = window.firmaProcessor.getSignatureForPDF('firmante1');
                const firmaFirmante2 = window.firmaProcessor.getSignatureForPDF('firmante2');
                
                if (firmaFirmante1) {
                    doc.addImage(firmaFirmante1, 'PNG', leftX - 25, y - 35, 50, 20);
                    console.log('✅ Firma del firmante 1 agregada al PDF');
                }
                
                if (firmaFirmante2) {
                    doc.addImage(firmaFirmante2, 'PNG', rightX - 25, y - 35, 50, 20);
                    console.log('✅ Firma del firmante 2 agregada al PDF');
                }
                
                if (firmaFirmante1 || firmaFirmante2) {
                    doc.setFont('times', 'italic');
                    doc.setFontSize(8);
    ...
    generateFileName() {
        ...
        return `Declaracion_Jurada_No_Parentesco_${nombreFirmante1}_${nombreFirmante2}_${timestamp}.pdf`;
    }
    ...
    validateFormWithSignatures() {
        ...
        const rutFirmante1 = document.getElementById('rutFirmante1').value;
        if (rutFirmante1 && !ConfigUtils.validateRUT(rutFirmante1)) {
            issues.push('RUT del primer firmante inválido');
        }

        const rutFirmante2 = document.getElementById('rutFirmante2').value;
        if (rutFirmante2 && !ConfigUtils.validateRUT(rutFirmante2)) {
            issues.push('RUT del segundo firmante inválido');
        }

        const cedulaFirmante1 = document.getElementById('cedulaFirmante1').value;
        if (cedulaFirmante1 && !ConfigUtils.validateCedula(cedulaFirmante1)) {
            issues.push('Cédula del primer firmante inválida');
        }

        const cedulaFirmante2 = document.getElementById('cedulaFirmante2').value;
        if (cedulaFirmante2 && !ConfigUtils.validateCedula(cedulaFirmante2)) {
            issues.push('Cédula del segundo firmante inválida');
        }
        ...
        const fechaNacimiento1 = document.getElementById('fechaNacimientoFirmante1').value;
        if (fechaNacimiento1 && !ConfigUtils.validateBirthDate(fechaNacimiento1)) {
            issues.push('Fecha de nacimiento del primer firmante inválida');
        }

        const fechaNacimiento2 = document.getElementById('fechaNacimientoFirmante2').value;
        if (fechaNacimiento2 && !ConfigUtils.validateBirthDate(fechaNacimiento2)) {
            issues.push('Fecha de nacimiento del segundo firmante inválida');
        }
        
        // Validar ciudad
        const ciudadDocumento = document.getElementById('ciudadDocumento').value;
        if (ciudadDocumento && !ConfigUtils.validateCity(ciudadDocumento)) {
            issues.push('Ciudad inválida');
        }
        
        if (!systemState.firmasDigitales.firmante1) {
            issues.push('Falta firma digital del primer firmante');
        }
        
        if (!systemState.firmasDigitales.firmante2) {
            issues.push('Falta firma digital del segundo firmante');
        }
...
class FirmaDigitalProcessor {
    ...
    applySignature() {
        if (this.currentType && this.processedImageData) {
            ...
            if (window.declaracionJuradaNoParentescoSystem) {
                window.declaracionJuradaNoParentescoSystem.updateProgress();
            }
            
            this.closeModal();
            
            if (window.declaracionJuradaNoParentescoSystem) {
                const tipoTexto = this.currentType === 'firmante1' ? 'del primer firmante' : 'del segundo firmante';
                window.declaracionJuradaNoParentescoSystem.showNotification(`Firma digital ${tipoTexto} aplicada correctamente`, 'success');
            }
        }
    }
    ...
    clearSignatures() {
        this.firmasDigitales = { firmante1: null, firmante2: null };
        
        systemState.firmasDigitales = { firmante1: false, firmante2: false };
        
        ['firmante1', 'firmante2'].forEach(type => {
            ...
            if (button) {
                ...
                const textElement = button.querySelector('.signature-text');
                if (textElement) {
                    textElement.textContent = type === 'firmante1' ? 'FIRMA FIRMANTE 1' : 'FIRMA FIRMANTE 2';
                }
            }
...
class PhotoCarnetDualProcessor {
    ...
    handleCameraError(error) {
        ...
        if (window.declaracionJuradaNoParentescoSystem) {
            window.declaracionJuradaNoParentescoSystem.showNotification(errorMessage, 'error');
        }
    }
    ...
    capturePhoto() {
        ...
        try {
            ...
            if (window.declaracionJuradaNoParentescoSystem) {
                window.declaracionJuradaNoParentescoSystem.showNotification(`Foto del ${this.currentStep} capturada`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al capturar foto:', error);
            if (window.declaracionJuradaNoParentescoSystem) {
                window.declaracionJuradaNoParentescoSystem.showNotification('Error al capturar la foto', 'error');
            }
        }
    ...
    nextStep() {
        if (this.currentStep === 'frente' && this.tempImages.frente) {
            ...
            this.showCamera();
            if (window.declaracionJuradaNoParentescoSystem) {
                window.declaracionJuradaNoParentescoSystem.showNotification('Ahora capture el reverso del carnet', 'info');
            }
        } else if (this.currentStep === 'reverso' && this.tempImages.reverso) {
            this.currentStep = 'completed';
            this.showSummary();
            if (window.declaracionJuradaNoParentescoSystem) {
                window.declaracionJuradaNoParentescoSystem.showNotification('Ambas fotos capturadas', 'success');
            }
        }
    }
    ...
    restartProcess() {
        ...
        this.showCamera();
        if (window.declaracionJuradaNoParentescoSystem) {
            window.declaracionJuradaNoParentescoSystem.showNotification('Proceso reiniciado', 'info');
        }
    }
    ...
    async confirmAllPhotos() {
        if (!this.tempImages.frente || !this.tempImages.reverso || !this.currentType) {
            if (window.declaracionJuradaNoParentescoSystem) {
                window.declaracionJuradaNoParentescoSystem.showNotification('Faltan fotos por capturar', 'error');
            }
            return;
        }
        ...
        try {
            ...
            if (window.declaracionJuradaNoParentescoSystem) {
                const tipoPersona = this.currentType === 'firmante1' ? 'primer firmante' : 'segundo firmante';
                window.declaracionJuradaNoParentescoSystem.showNotification(`Carnet completo del ${tipoPersona} guardado`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al confirmar fotos:', error);
            if (window.declaracionJuradaNoParentescoSystem) {
                window.declaracionJuradaNoParentescoSystem.showNotification('Error al procesar las fotos', 'error');
            }
        }
    }
...
window.clearForm = function() {
    if (window.declaracionJuradaNoParentescoSystem) {
        window.declaracionJuradaNoParentescoSystem.clearForm();
    }
};

window.generatePDF = function() {
    if (window.declaracionJuradaNoParentescoSystem) {
        window.declaracionJuradaNoParentescoSystem.generatePDF();
    }
};

// ===== INICIALIZACIÓN DEL SISTEMA =====
let declaracionJuradaNoParentescoSystem;

function initializeDeclaracionJuradaNoParentescoSystem() {
    console.log('📋 Iniciando Sistema de Declaración Jurada No Parentesco...');
    
    // Verificar que jsPDF esté disponible
    if (typeof window.jspdf === 'undefined') {
        console.warn('⚠️ jsPDF no está disponible, esperando...');
        setTimeout(initializeDeclaracionJuradaNoParentescoSystem, 1000);
        return;
    }
    ...
    try {
        declaracionJuradaNoParentescoSystem = new DeclaracionJuradaNoParentescoSystem();
        window.declaracionJuradaNoParentescoSystem = declaracionJuradaNoParentescoSystem;
        
        console.log('✅ Sistema completamente inicializado');
    } catch (error) {
        console.error('❌ Error crítico en inicialización:', error);
        setTimeout(initializeDeclaracionJuradaNoParentescoSystem, 2000);
    }
}

// ===== MÚLTIPLES PUNTOS DE INICIALIZACIÓN =====
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeDeclaracionJuradaNoParentescoSystem);
} else if (document.readyState === 'interactive' || document.readyState === 'complete') {
    setTimeout(initializeDeclaracionJuradaNoParentescoSystem, 500);
}
...
window.addEventListener('error', (event) => {
    ...
    if (window.declaracionJuradaNoParentescoSystem) {
        window.declaracionJuradaNoParentescoSystem.showNotification(
...
// ===== LOG FINAL =====
console.log('📜 Sistema de Declaración Jurada No Parentesco - Script principal cargado correctamente');