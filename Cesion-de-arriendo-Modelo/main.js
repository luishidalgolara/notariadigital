// ========================================
// 📋 SISTEMA PRINCIPAL DE CESIÓN DE ARRENDAMIENTO
// ========================================
...
const systemState = {
    formFields: [
        ...
        'nombresInvitante', 'nacionalidadInvitante', 'runInvitante', 'domicilioInvitante', 'ocupacionInvitante', 'telefonoInvitante', 'estadoCivilArrendador', 'comunaCedente', 'regionCedente',
        'nombresInvitado', 'nacionalidadInvitado', 'rutRepresentante', 'estadoCivilRepresentante', 'profesionCesionario', 'domicilioCesionario', 'comunaCesionario', 'regionCesionario',
        'descripcionInmueble', 'direccionInmueble', 'accesoPrincipal', 'nombreCondominio', 'comunaInmueble', 'regionInmueble', 'numeroFojas', 'anioRegistro', 'conservadorBienes',
        'fechaContratoOriginal', 'tipoDocumentoOriginal',
        'montoGarantia', 'garantiaPalabras', 'rentaMensual', 'rentaUF', 'chequesPendientes', 'montoCheque', 'cuentaCorriente', 'nombreBanco', 'emailComprobantes',
        'nombreArrendatario', 'rutArrendatario', 'nacionalidadArrendatario', 'estadoCivilArrendatario', 'profesionArrendatario', 'domicilioArrendatario',
        'nombreAval', 'rutAval', 'observacionesAdicionales'
    ],
...
class CesionArriendoSystem {
    ...
    async init() {
        console.log('📋 Inicializando Sistema de Cesión de Arrendamiento...');
    ...
    setupSpecialFieldListeners() {
        ...
        // Validación RUT Arrendatario
        const rutArrendatario = document.getElementById('rutArrendatario');
        if (rutArrendatario) {
            rutArrendatario.addEventListener('input', (e) => {
                this.handleRutInput(e, 'rutArrendatario');
            });
        }

        // Validación RUT Aval
        const rutAval = document.getElementById('rutAval');
        if (rutAval) {
            rutAval.addEventListener('input', (e) => {
                this.handleRutInput(e, 'rutAval');
            });
        }
        ...
        // Convertir garantía a palabras
        const montoGarantia = document.getElementById('montoGarantia');
        const garantiaPalabras = document.getElementById('garantiaPalabras');
        if (montoGarantia && garantiaPalabras) {
            montoGarantia.addEventListener('input', (e) => {
                const valor = parseInt(e.target.value);
                if (!isNaN(valor) && valor > 0) {
                    const palabras = NumberToWords.convertToWords(valor);
                    garantiaPalabras.value = palabras.charAt(0).toUpperCase() + palabras.slice(1);
                } else {
                    garantiaPalabras.value = '';
                }
                this.updatePreview();
            });
        }
        ...
        // Validar email
        const emailComprobantes = document.getElementById('emailComprobantes');
        if (emailComprobantes) {
            emailComprobantes.addEventListener('input', (e) => {
    ...
    updateDuplicateReferences() {
        // Referencias múltiples del cedente
        ...
        // Referencias múltiples del cesionario
        ...
        const conservadorBienes = document.getElementById('conservadorBienes').value;
        
        // Actualizar todas las referencias
        const updates = {
            'prev_nombresInvitante2': nombresInvitante || '_________________________',
            'prev_nombresInvitante3': nombresInvitante || '_________________________',
            'prev_nombresInvitante4': nombresInvitante || '_________________________',
            'prev_nombresInvitante5': nombresInvitante || '_________________________',
            'prev_runInvitante2': runInvitante || '___________',
            'prev_nombresInvitado2': nombresInvitado || '_________________________',
            'prev_nombresInvitado3': nombresInvitado || '_________________________',
            'prev_nombresInvitado4': nombresInvitado || '_________________________',
            'prev_nombresInvitado5': nombresInvitado || '_________________________',
            'prev_nombresInvitado6': nombresInvitado || '_________________________',
            'prev_nombresInvitado7': nombresInvitado || '_________________________',
            'prev_ciudadCarta2': ciudadCarta || 'SANTIAGO',
            'prev_conservadorBienes2': conservadorBienes || '________'
        };
    ...
    getPlaceholderText(field) {
        const placeholders = {
            ...
            'domicilioCesionario': '_________________________',
            'domicilioArrendatario': '_________________________',
            'ocupacionInvitante': '________',
            'profesionCesionario': '________',
            'profesionArrendatario': '________',
            'telefonoInvitante': '___________',
            'comunaCedente': '________',
            'comunaCesionario': '________',
            'regionCedente': 'Metropolitana',
            'regionCesionario': 'Metropolitana',
            'regionInmueble': 'Metropolitana',
            'descripcionInmueble': '________',
            'direccionInmueble': '_________________________',
            'accesoPrincipal': '_________________________',
            'nombreCondominio': '________',
            'comunaInmueble': '________',
            'numeroFojas': '________',
            'anioRegistro': '________',
            'conservadorBienes': '________',
            'fechaContratoOriginal': '________',
            'tipoDocumentoOriginal': '________',
            'montoGarantia': '________',
            'garantiaPalabras': '________',
            'rentaMensual': '________',
            'rentaUF': '________',
            'chequesPendientes': '________',
            'montoCheque': '________',
            ...
            'estadoCivilArrendatario': '________',
            'nombreAval': '_________________________',
    ...
    openSignatureModalSafe(type) {
        ...
        if (!nombreField || !nombreField.value.trim()) {
            this.showNotification('Complete el nombre del cedente antes de firmar', 'warning');
            return;
        }
    ...
    openPhotoModalSafe(type) {
        ...
        if (!nombreField || !nombreField.value.trim()) {
            this.showNotification('Complete el nombre del cedente antes de tomar fotos', 'warning');
            return;
        }
    ...
    initializeFormFields() {
        ...
        // Valores por defecto para regiones
        const regionFields = ['regionCedente', 'regionCesionario', 'regionInmueble'];
        regionFields.forEach(fieldId => {
            const field = document.getElementById(fieldId);
            if (field && !field.value) {
                field.value = 'Metropolitana';
            }
        });
    ...
    clearForm() {
        this.showConfirmDialog(
            ...
            () => {
                ...
                // Restaurar valores por defecto para regiones
                const regionFields = ['regionCedente', 'regionCesionario', 'regionInmueble'];
                regionFields.forEach(fieldId => {
                    const field = document.getElementById(fieldId);
                    if (field) {
                ...
                const ciudadCarta = document.getElementById('ciudadCarta');
                if (ciudadCarta) {
                    ciudadCarta.value = 'Santiago';
                }
    ...
    async generatePDF() {
        ...
        try {
            ...
            setTimeout(() => {
                doc.save(fileName);
                this.showNotification('PDF de Cesión de Arrendamiento generado exitosamente', 'success');
            }, 1500);
    ...
    async addPDFContent(doc) {
        ...
        doc.setFontSize(18);
        doc.text('CESIÓN DE ARRENDAMIENTO', 105, y, { align: 'center' });
        y += 30;

        // Subtítulo con partes
        doc.setFont('times', 'bold');
        doc.setFontSize(14);
        const nombresInvitante = document.getElementById('nombresInvitante').value || 'CEDENTE';
        const nombresInvitado = document.getElementById('nombresInvitado').value || 'CESIONARIO';
        
        doc.text(nombresInvitante, 105, y, { align: 'center' });
        y += 15;
        doc.text('A', 105, y, { align: 'center' });
        y += 15;
        doc.text(nombresInvitado, 105, y, { align: 'center' });
        y += 30;
        ...
        const comunaCedente = document.getElementById('comunaCedente').value;
        const regionCedente = document.getElementById('regionCedente').value;
        ...
        const parrafo1 = `EN ${ciudadCarta.toUpperCase()} DE CHILE, a ${fechaTexto}, comparecen: Don ${nombresInvitante}, ${nacionalidadInvitante}, ${estadoCivilArrendador}, ${ocupacionInvitante}, Cédula de Identidad número ${runInvitante}, con domicilio en ${domicilioInvitante}, Comuna de ${comunaCedente}, Región ${regionCedente}, en adelante también denominado indistintamente como el "Cedente" o "Propietario"; y por la otra parte, en adelante también denominado indistintamente como el "Cesionario" y "Compradora", doña ${nombresInvitado}, quien han convenido libre y voluntariamente, en pleno uso de sus facultades, en celebrar el siguiente Contrato de Cesión de arrendamiento:`;
        ...
        // Cláusulas adicionales del contrato de cesión...
        const fechaContratoOriginal = document.getElementById('fechaContratoOriginal').value;
        const tipoDocumentoOriginal = document.getElementById('tipoDocumentoOriginal').value;
        const descripcionInmueble = document.getElementById('descripcionInmueble').value;

        let fechaContratoTexto = '________';
        if (fechaContratoOriginal && ConfigUtils.validateDate(fechaContratoOriginal)) {
            const date = new Date(fechaContratoOriginal);
            fechaContratoTexto = `${date.getDate()} de ${this.mesesEspanol[date.getMonth()]} del ${date.getFullYear()}`;
        }

        const clausula1 = `PRIMERO: Don ${nombresInvitante}, suscribió con fecha ${fechaContratoTexto}, por ${tipoDocumentoOriginal}, un contrato de arrendamiento de ${descripcionInmueble}.`;
        ...
        const garantiaPalabras = document.getElementById('garantiaPalabras').value;
        const rentaMensual = document.getElementById('rentaMensual').value;
        const rentaUF = document.getElementById('rentaUF').value;

        const clausula4 = `CUARTO: Don ${nombresInvitante} viene en este acto en hacer entrega de la suma de $${montoGarantia} (${garantiaPalabras}) pesos, suma que corresponde a la garantía de fiel cumplimiento del arrendamiento.`;
        
        doc.text(clausula4, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausula4, pageWidth - 2 * margin) + 15;

        const clausula5 = `QUINTO: La renta de arrendamiento a partir del mes siguiente de la firma de este contrato asciende a la suma de $${rentaMensual} pesos, equivalentes a ${rentaUF} UF.`;
    ...
    addSignatureSection(doc, y) {
        ...
        doc.setFont('times', 'normal');
        doc.setFontSize(10);
        doc.text('CEDENTE - VENDEDOR', centerX, y, { align: 'center' });
        y += 8;
        ...
        if (window.firmaProcessor) {
            try {
                ...
                if (firmaInvitante) {
                    ...
                    console.log('✅ Firma del cedente agregada al PDF');
    ...
    generateFileName() {
        ...
        return `Cesion_Arrendamiento_${nombresInvitante}_${nombresInvitado}_${timestamp}.pdf`;
    }
    ...
    validateFormWithSignatures() {
        ...
        const requiredFields = [
            ...
            'descripcionInmueble', 'direccionInmueble', 'comunaInmueble',
            'fechaContratoOriginal', 'tipoDocumentoOriginal',
            'montoGarantia', 'rentaMensual'
        ];
        ...
        const rutAval = document.getElementById('rutAval').value;
        
        if (runInvitante && !ConfigUtils.validateRUT(runInvitante)) {
            issues.push('RUT del cedente inválido');
        }
        
        if (rutRepresentante && !ConfigUtils.validateRUT(rutRepresentante)) {
            issues.push('RUT del cesionario inválido');
        }
        
        if (rutArrendatario && !ConfigUtils.validateRUT(rutArrendatario)) {
            issues.push('RUT del arrendatario inválido');
        }
        
        if (rutAval && rutAval.trim() && !ConfigUtils.validateRUT(rutAval)) {
            issues.push('RUT del aval inválido');
        }
        ...
        const emailComprobantes = document.getElementById('emailComprobantes').value;
        if (emailComprobantes && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailComprobantes)) {
            issues.push('Email para comprobantes inválido');
        }
        
        if (!systemState.firmasDigitales.invitante) {
            issues.push('Falta firma digital del cedente');
        }
...
// ===== CLASES DE PROCESADORES (MANTENER IGUALES) =====
class FirmaDigitalProcessor {
    ...
    applySignature() {
        if (this.currentType && this.processedImageData) {
            ...
            if (window.cesionArriendoSystem) {
                window.cesionArriendoSystem.updateProgress();
            }
            
            this.closeModal();
            
            if (window.cesionArriendoSystem) {
                window.cesionArriendoSystem.showNotification(`Firma digital del ${this.currentType} aplicada correctamente`, 'success');
            }
        }
    }
...
// ===== CLASE PROCESADOR DE FOTOS (MANTENER IGUAL) =====
class PhotoCarnetDualProcessor {
    ...
    handleCameraError(error) {
        ...
        if (window.cesionArriendoSystem) {
            window.cesionArriendoSystem.showNotification(errorMessage, 'error');
        }
    }
    ...
    capturePhoto() {
        ...
        try {
            ...
            if (window.cesionArriendoSystem) {
                window.cesionArriendoSystem.showNotification(`Foto del ${this.currentStep} capturada`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al capturar foto:', error);
            if (window.cesionArriendoSystem) {
                window.cesionArriendoSystem.showNotification('Error al capturar la foto', 'error');
            }
        }
    ...
    nextStep() {
        if (this.currentStep === 'frente' && this.tempImages.frente) {
            ...
            this.showCamera();
            if (window.cesionArriendoSystem) {
                window.cesionArriendoSystem.showNotification('Ahora capture el reverso del carnet', 'info');
            }
        } else if (this.currentStep === 'reverso' && this.tempImages.reverso) {
            this.currentStep = 'completed';
            this.showSummary();
            if (window.cesionArriendoSystem) {
                window.cesionArriendoSystem.showNotification('Ambas fotos capturadas', 'success');
            }
        }
    }
    ...
    restartProcess() {
        ...
        this.showCamera();
        if (window.cesionArriendoSystem) {
            window.cesionArriendoSystem.showNotification('Proceso reiniciado', 'info');
        }
    }
    ...
    async confirmAllPhotos() {
        if (!this.tempImages.frente || !this.tempImages.reverso || !this.currentType) {
            if (window.cesionArriendoSystem) {
                window.cesionArriendoSystem.showNotification('Faltan fotos por capturar', 'error');
            }
            return;
        }
        ...
        try {
            ...
            if (window.cesionArriendoSystem) {
                window.cesionArriendoSystem.showNotification(`Carnet completo del ${this.currentType} guardado`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al confirmar fotos:', error);
            if (window.cesionArriendoSystem) {
                window.cesionArriendoSystem.showNotification('Error al procesar las fotos', 'error');
            }
        }
    }
...
window.clearForm = function() {
    if (window.cesionArriendoSystem) {
        window.cesionArriendoSystem.clearForm();
    }
};

window.generatePDF = function() {
    if (window.cesionArriendoSystem) {
        window.cesionArriendoSystem.generatePDF();
    }
};

// ===== INICIALIZACIÓN DEL SISTEMA =====
let cesionArriendoSystem;

function initializeCesionArriendoSystem() {
    console.log('📋 Iniciando Sistema de Cesión de Arrendamiento...');
    
    // Verificar que jsPDF esté disponible
    if (typeof window.jspdf === 'undefined') {
        console.warn('⚠️ jsPDF no está disponible, esperando...');
        setTimeout(initializeCesionArriendoSystem, 1000);
        return;
    }
    ...
    try {
        cesionArriendoSystem = new CesionArriendoSystem();
        window.cesionArriendoSystem = cesionArriendoSystem;
        
        console.log('✅ Sistema completamente inicializado');
    } catch (error) {
        console.error('❌ Error crítico en inicialización:', error);
        setTimeout(initializeCesionArriendoSystem, 2000);
    }
}

// ===== MÚLTIPLES PUNTOS DE INICIALIZACIÓN =====
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeCesionArriendoSystem);
} else if (document.readyState === 'interactive' || document.readyState === 'complete') {
    setTimeout(initializeCesionArriendoSystem, 500);
}
...
window.addEventListener('error', (event) => {
    ...
    if (window.cesionArriendoSystem) {
        window.cesionArriendoSystem.showNotification(
...
// ===== LOG FINAL =====
console.log('📜 Sistema de Cesión de Arrendamiento - Script principal cargado correctamente');