...
// ========================================
// Archivo único auto-contenido - ADAPTADO para Aviso de Término de Contrato
// Versión: 2.0.0 | Compatible con empleador/trabajador
...
(function() {
    ...
    function injectCSS() {
        ...
        style.textContent = `
            ...
            .face-modal-container {
                ...
                box-shadow: 0 25px 50px rgba(0, 0, 0, 0.7), 0 0 0 1px rgba(59, 130, 246, 0.1);
    ...
    class FaceVerificationSystem {
        constructor() {
            ...
            this.verificationData = {
                empleador: { verified: false, timestamp: null, data: null },
                trabajador: { verified: false, timestamp: null, data: null }
            };
        ...
        checkBrowserCompatibility() {
            ...
            if (location.protocol !== 'https:' && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
                this.showNotification('⚠️ Se requiere HTTPS para acceder a la cámara. Por favor use una conexión segura.', 'warning');
                return false;
            }

            // Verificar soporte de getUserMedia
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                this.showNotification('❌ Su navegador no soporta acceso a cámara. Use Chrome, Firefox o Safari reciente.', 'error');
                return false;
            }
        ...
        validateUserData(type) {
            // ADAPTADO: Buscar campos empleador/trabajador en lugar de vendedor/comprador
            let nombreField;
            if (type === 'empleador') {
            ...
            if (!nombreField || !nombreField.value.trim()) {
                ...
                this.showNotification(`Complete el nombre del ${entityName} antes de la verificación facial`, 'warning');
                return false;
            }
        ...
        resetState() {
            ...
            // Resetear UI pero NO ocultar modal
        ...
        confirmVerification() {
            ...
            if (window.avisoTerminoSystem && window.avisoTerminoSystem.updateProgress) {
        ...
        updateButton(type, verified, skipped = false) {
            ...
            if (button) {
                ...
                if (verified) {
                    if (skipped) {
                        ...
                        if (textEl) textEl.textContent = 'OMITIDO';
                    } else {
                        button.classList.add('verified');
        ...
        handleCameraError(error) {
            ...
            switch (error.name) {
                case 'NotAllowedError':
                    errorTitle = '🚫 Acceso a la Cámara Denegado';
                    errorMessage = 'Necesitamos acceso a su cámara para la verificación facial';
                    instructions = [
                        '1. Haga clic en el icono de cámara 📷 en la barra de direcciones',
                        '2. Seleccione "Permitir" o "Allow"',
                        '3. Recargue la página e intente nuevamente',
                        '4. Asegúrese de estar en una conexión HTTPS segura'
                    ];
                    break;
                    
                case 'NotFoundError':
                    errorTitle = '📷 Cámara No Encontrada';
                    errorMessage = 'No se detectó ninguna cámara en su dispositivo';
                    instructions = [
                        '1. Verifique que su cámara esté conectada',
                        '2. Cierre otras aplicaciones que puedan estar usando la cámara',
                        '3. Intente usar otro navegador o dispositivo'
                    ];
                    break;
                ...
                case 'NotReadableError':
                    ...
                    instructions = [
                        ...
                        '3. Reinicie el navegador e intente nuevamente'
                    ];
                    break;
                ...
                default:
                    ...
                    instructions = [
                        '1. Recargue la página e intente nuevamente',
                        '2. Verifique que esté en HTTPS',
            ...
            this.showDetailedError(errorTitle, errorMessage, instructions);
            
            // Mostrar notificación también
            this.showNotification(errorMessage, 'error');
            
            console.log(`🚫 Error de cámara manejado: ${error.name} - No cerrando modal`);
        }

        showDetailedError(title, message, instructions) {
            console.log(`🔧 Mostrando error detallado: ${title}`);
            ...
            if (loading) {
                loading.innerHTML = `
                    <div style="text-align: center; padding: 30px; max-width: 500px; margin: 0 auto;">
                        ...
                        <h3 style="color: #ef4444; margin-bottom: 15px; font-size: 1.4rem;">${title}</h3>
                        <p style="color: #e2e8f0; margin-bottom: 25px; font-size: 1.1rem; line-height: 1.5;">${message}</p>
                        
                        <div style="text-align: left; background: rgba(51, 65, 85, 0.5); padding: 20px; border-radius: 12px; margin-bottom: 25px;">
                            <h4 style="color: #3b82f6; margin-bottom: 15px; font-size: 1rem; text-align: center;">📋 Cómo solucionarlo:</h4>
                            ${instructions.map(instruction => `
                                <div style="color: #cbd5e0; margin-bottom: 12px; font-size: 0.9rem; line-height: 1.5; padding: 8px 0;">
                                    ${instruction}
                                </div>
                            `).join('')}
                        </div>
                        ...
                        <div style="display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
                            <button class="face-btn face-btn-retry" style="background: linear-gradient(135deg, #10b981, #059669); border: none; color: white; padding: 12px 24px; border-radius: 8px; cursor: pointer; font-weight: 600; display: flex; align-items: center; gap: 8px;" onclick="window.faceVerificationSystem.retryCamera()">
                                <span>🔄</span>
                                <span>Reintentar</span>
                            </button>
                            <button class="face-btn face-btn-confirm" style="background: linear-gradient(135deg, #3b82f6, #2563eb); border: none; color: white; padding: 12px 24px; border-radius: 8px; cursor: pointer; font-weight: 600; display: flex; align-items: center; gap: 8px;" onclick="window.faceVerificationSystem.skipVerification()">
                                <span>⏭️</span>
                                <span>Omitir</span>
                            </button>
                        </div>
                        
                        <p style="color: #94a3b8; font-size: 0.8rem; margin-top: 20px; line-height: 1.4;">
                            💡 Consejo: La verificación facial es opcional. Puede omitirla y continuar con el documento.
                        </p>
                    </div>
                `;
                loading.classList.add('active');
            ...
            console.log(`✅ Error mostrado correctamente en modal`);
        }

        retryCamera() {
            console.log('🔄 Reintentando acceso a cámara...');
            
            // Resetear el estado pero mantener el modal abierto
            this.currentInstructionIndex = 0;
            this.completedInstructions = [];
            this.detectionFrames = 0;
            this.isRecording = false;
            ...
            this.showLoading('Reintentando acceso a cámara...', 'Solicitando permisos nuevamente...');
            
            // Intentar acceder a la cámara después de un breve delay
            setTimeout(() => {
                this.initializeCamera();
            }, 1000);
        }
        ...
        skipVerification() {
            console.log('⏭️ Omitiendo verificación facial...');
            
            // Marcar como "verificado" pero con flag especial
            this.verificationData[this.currentType] = {
                verified: true,
                skipped: true,
                timestamp: new Date().toISOString(),
            ...
            // Actualizar botón con estado "omitido"
            this.updateButton(this.currentType, true, true);
            ...
            this.showNotification(`Verificación facial omitida para ${this.currentType}. El documento se generará sin este requisito.`, 'info');
        }

        showNotification(message, type = 'info') {
            // ADAPTADO: Usar el sistema correcto (avisoTerminoSystem en lugar de promesaSystem)
            if (window.avisoTerminoSystem && window.avisoTerminoSystem.showNotification) {
                window.avisoTerminoSystem.showNotification(message, type);
            } else {
        ...
        clearAllVerifications() {
            this.clearVerification('empleador');
            this.clearVerification('trabajador');
            console.log('🧹 Todas las verificaciones faciales eliminadas');
        }
    }

    // ===== FUNCIONES DE INTEGRACIÓN ADAPTADAS =====
    function addFaceVerificationButtons() {
        const buttonGroups = document.querySelectorAll('.button-group');
        
        buttonGroups.forEach((group, index) => {
            // ADAPTADO: Identificar el tipo basado en los botones existentes
            const firmaBtn = group.querySelector('[id*="firma"]');
            let type = 'empleador';
            
            if (firmaBtn && firmaBtn.id.includes('Trabajador')) {
                type = 'trabajador';
            }
    ...
    function integrateClearForm() {
        // ADAPTADO: Buscar la función clearForm global
        ...
        // ADAPTADO: También buscar en el sistema principal (avisoTerminoSystem)
        if (window.avisoTerminoSystem && window.avisoTerminoSystem.clearForm) {
            const originalMethod = window.avisoTerminoSystem.clearForm;
            window.avisoTerminoSystem.clearForm = function() {
                if (window.faceVerificationSystem) {
                    window.faceVerificationSystem.clearAllVerifications();
                }
                return originalMethod.call(this);
            };
            console.log('🔗 Integración con avisoTerminoSystem.clearForm completada');
        }
    }
    ...
    function initializeFaceVerification() {
        ...
        console.log('📋 Integración con sistema de Aviso de Término de Contrato completada');
    }
    ...
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        window.debugFaceVerification = {
            ...
            openModal: (type) => window.faceVerificationSystem?.openModal(type || 'empleador'),
...
✅ Versión: 2.0.0 - ADAPTADO PARA CONTRATOS
✅ Integración: Aviso de Término de Contrato
✅ Compatibilidad: Sistema laboral adaptado
✅ Anti-Robot: Detección de vida activada
✅ Usuarios: Empleador y Trabajador
✅ Responsive: Compatible con dispositivos

🔧 Adaptaciones aplicadas:
   • Campos actualizados: empleador/trabajador
   • Validación de nombres corregida
   • Integración con avisoTerminoSystem
   • Botones auto-detectados correctamente
   • Colores adaptados al tema azul
