// ========================================
// 📋 CONFIGURACIÓN DEL SISTEMA DE CONTRATO DE TRABAJO INDEFINIDO
// ========================================

const CONFIG = {
    // ===== CONFIGURACIÓN GENERAL =====
    APP: {
        name: 'Notaría Digital - Contrato de Trabajo Indefinido',
        version: '1.0.0',
        author: 'Sistema Notarial Digital Chile',
        description: 'Sistema profesional para generar contratos de trabajo indefinido con firmas digitales'
    },
    ...
    MESSAGES: {
        ...
        error: {
            ...
            invalid_salary_amount: 'Monto de sueldo inválido',
            invalid_dates: 'Las fechas son inválidas',
            invalid_hours: 'Horario de trabajo inválido',
    ...
    // ===== CONFIGURACIÓN ESPECÍFICA DE TRABAJO =====
    WORK: {
        positions: [
            'Desarrollador de Software', 'Contador', 'Secretaria', 'Vendedor', 'Gerente',
            'Analista', 'Recepcionista', 'Técnico', 'Supervisor', 'Coordinador', 
            'Asistente', 'Ejecutivo', 'Operador', 'Especialista', 'Otro'
        ],
        work_days: [
    ...
    CONTRACT: {
        payment_frequencies: [
            ...
            'el día 15 y último día del mes'
        ],
        overtime_options: [
            'si', 'no'
        ],
        gratification_options: [
            'si', 'no'
        ]
    }
};
...
const ConfigUtils = {
    ...
    // Validar monto de sueldo
    validateSalaryAmount(amount) {
        if (!amount || isNaN(amount)) return false;
        
        const numAmount = parseFloat(amount);
        return numAmount >= CONFIG.VALIDATION.salary_amount.min &&
               numAmount <= CONFIG.VALIDATION.salary_amount.max;
    },
    ...
    validateWeeklyHours(hours) {
        ...
        return numHours >= CONFIG.VALIDATION.weekly_hours.min &&
               numHours <= CONFIG.VALIDATION.weekly_hours.max;
    },
    ...
    validateWorkDays(days) {
        ...
        return numDays >= CONFIG.VALIDATION.work_days.min &&
               numDays <= CONFIG.VALIDATION.work_days.max;
    },
    ...
    validateBreakMinutes(minutes) {
        ...
        return numMinutes >= CONFIG.VALIDATION.break_minutes.min &&
               numMinutes <= CONFIG.VALIDATION.break_minutes.max;
    },
    ...
    validateWeekDay(day) {
        return CONFIG.WORK.work_days.includes(day.toLowerCase());
    }
};
...
ConfigUtils.log('info', 'Sistema de configuración de Contrato de Trabajo Indefinido inicializado', {
