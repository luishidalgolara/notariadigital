// ========================================
// 📋 SISTEMA PRINCIPAL DE AUTORIZACIÓN NOTARIAL DE SALIDA DE VEHÍCULO
// ========================================
...
class CartaInvitacionSystem {
    ...
    openSignatureModalSafe(type) {
        ...
        if (!nombreField || !nombreField.value.trim()) {
            this.showNotification('Complete el nombre del autorizante antes de firmar', 'warning');
            return;
        }
    ...
    openPhotoModalSafe(type) {
        ...
        if (!nombreField || !nombreField.value.trim()) {
            this.showNotification('Complete el nombre del autorizante antes de tomar fotos', 'warning');
            return;
        }
    ...
    async addPDFContent(doc) {
        ...
        doc.setFont('times', 'normal');
        doc.text(anioVehiculo, margin + 25, y);
        y += 8;
...
function initializeCartaInvitacionSystem() {
    console.log('📋 Iniciando Sistema de Autorización Notarial de Salida de Vehículo...');
...
// ===== LOG FINAL =====
console.log('📜 Sistema de Autorización Notarial de Salida de Vehículo - Script principal cargado correctamente');