...
// ========================================
// Archivo único auto-contenido - CORREGIDO para Poder Simple
// Versión: 1.0.1 | Compatible con mandante/mandatario
...
(function() {
    ...
    class FaceVerificationSystem {
        constructor() {
            ...
            this.verificationData = {
                mandante: { verified: false, timestamp: null, data: null },
                mandatario: { verified: false, timestamp: null, data: null }
            };
        ...
        validateUserData(type) {
            // CORREGIDO: Buscar campos mandante/mandatario en lugar de vendedor/comprador
            const nombreField = document.getElementById(`nombre${type.charAt(0).toUpperCase() + type.slice(1)}`);
            
            if (!nombreField || !nombreField.value.trim()) {
                this.showNotification(`Complete el nombre del ${type} antes de la verificación facial`, 'warning');
                return false;
            }
        ...
        confirmVerification() {
            ...
            if (window.poderSystem && window.poderSystem.updateProgress) {
        ...
        handleCameraError(error) {
            ...
            switch (error.name) {
                ...
                case 'OverconstrainedError':
                    ...
                    errorMessage = 'Su cámara no soporta la resolución requerida';
                    instructions = [
                        '1. Intente con otro dispositivo o cámara',
                        '2. Actualice los drivers de su cámara',
                        '3. Use un navegador más reciente'
                    ];
                    break;
                    
                default:
                    errorTitle = '❌ Error Desconocido';
                    errorMessage = 'Ocurrió un error inesperado con la cámara';
                    instructions = [
            ...
            if (loading) {
                loading.innerHTML = `
                    <div style="text-align: center; padding: 30px; max-width: 500px; margin: 0 auto;">
                        ...
                        <h3 style="color: #f56565; margin-bottom: 15px; font-size: 1.4rem;">${title}</h3>
                        <p style="color: #e2e8f0; margin-bottom: 25px; font-size: 1.1rem; line-height: 1.5;">${message}</p>
                        
                        <div style="text-align: left; background: rgba(45, 55, 72, 0.5); padding: 20px; border-radius: 12px; margin-bottom: 25px;">
                            <h4 style="color: #ed8936; margin-bottom: 15px; font-size: 1rem; text-align: center;">📋 Cómo solucionarlo:</h4>
                        ...
                        <div style="display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
                            <button class="face-btn face-btn-retry" style="background: linear-gradient(135deg, #48bb78, #38a169); border: none; color: white; padding: 12px 24px; border-radius: 8px; cursor: pointer; font-weight: 600; display: flex; align-items: center; gap: 8px;" onclick="window.faceVerificationSystem.retryCamera()">
                                <span>🔄</span>
                                <span>Reintentar</span>
                            </button>
                            <button class="face-btn face-btn-confirm" style="background: linear-gradient(135deg, #ed8936, #dd6b20); border: none; color: white; padding: 12px 24px; border-radius: 8px; cursor: pointer; font-weight: 600; display: flex; align-items: center; gap: 8px;" onclick="window.faceVerificationSystem.skipVerification()">
                                <span>⏭️</span>
                                <span>Omitir</span>
                            </button>
                        </div>
                        
                        <p style="color: #a0aec0; font-size: 0.8rem; margin-top: 20px; line-height: 1.4;">
        ...
        showNotification(message, type = 'info') {
            // CORREGIDO: Usar el sistema correcto (poderSystem en lugar de promesaSystem)
            if (window.poderSystem && window.poderSystem.showNotification) {
                window.poderSystem.showNotification(message, type);
            } else {
        ...
        clearAllVerifications() {
            this.clearVerification('mandante');
            this.clearVerification('mandatario');
    ...
    function addFaceVerificationButtons() {
        ...
        buttonGroups.forEach((group, index) => {
            // CORREGIDO: Identificar el tipo basado en los botones existentes
            const firmaBtn = group.querySelector('[id*="firma"]');
            let type = 'mandante';
            
            if (firmaBtn && firmaBtn.id.includes('Mandatario')) {
                type = 'mandatario';
            }
    ...
    function integrateClearForm() {
        // CORREGIDO: Buscar la función clearForm global
        ...
        // CORREGIDO: También buscar en el sistema principal (poderSystem)
        if (window.poderSystem && window.poderSystem.clearForm) {
            const originalMethod = window.poderSystem.clearForm;
            window.poderSystem.clearForm = function() {
                if (window.faceVerificationSystem) {
                    window.faceVerificationSystem.clearAllVerifications();
                }
                return originalMethod.call(this);
            };
            console.log('🔗 Integración con poderSystem.clearForm completada');
        }
    }
    ...
    function initializeFaceVerification() {
        ...
        console.log('🎯 Integración con sistema de Poder Simple completada');
    }
    ...
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        window.debugFaceVerification = {
            ...
            openModal: (type) => window.faceVerificationSystem?.openModal(type || 'mandante'),
