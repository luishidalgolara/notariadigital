// ========================================
// 🏛️ SISTEMA DE PODER SIMPLE - SCRIPT PRINCIPAL CORREGIDO
// ========================================
...
class PoderSimpleSystem {
    ...
    async init() {
        console.log('🏛️ Inicializando Sistema de Poder Simple...');
    ...
    // NUEVO: Esperar a que DOM esté completamente listo
    async waitForDOM() {
    ...
    // NUEVO: Asegurar que elementos críticos existan
    async ensureCriticalElements() {
        const criticalElements = [
            'firmaModalOverlay', 'photoModalOverlay',
            'firmaMandante', 'firmaMandatario',
            'photoMandante', 'photoMandatario',
    ...
    // NUEVO: Inicialización del núcleo del sistema
    ...
    // NUEVO: Inicialización de sistemas especiales con verificaciones
    ...
    // NUEVO: Reintentar sistemas fallidos
    ...
    setupSpecialFieldListeners() {
        ...
        const rutMandante = document.getElementById('rutMandante');
        if (rutMandante) {
            rutMandante.addEventListener('input', (e) => {
                this.handleRutInput(e, 'rutMandante');
            });
        }

        // Validación RUT Mandatario
        const rutMandatario = document.getElementById('rutMandatario');
        if (rutMandatario) {
            rutMandatario.addEventListener('input', (e) => {
                this.handleRutInput(e, 'rutMandatario');
            });
        }

        // Validación longitud efectos del poder
        const efectosPoder = document.getElementById('efectosPoder');
        if (efectosPoder) {
            efectosPoder.addEventListener('input', (e) => {
                this.handleTextInput(e);
            });
        }

        // Auto-llenar ciudad del poder cuando se cambia fecha
        const fechaPoder = document.getElementById('fechaPoder');
        if (fechaPoder) {
            fechaPoder.addEventListener('change', () => {
    ...
    handleTextInput(event) {
        ...
        const isValid = ConfigUtils.validateTextLength(text, 10, 2000);
        
        event.target.classList.toggle('rut-valid', isValid && text.length >= 10);
        event.target.classList.toggle('rut-invalid', text.length > 0 && text.length < 10);
        
        this.updatePreview();
    }

    handleDateChange() {
        const fecha = document.getElementById('fechaPoder').value;
        if (fecha && ConfigUtils.validateDate(fecha)) {
            // Auto-completar día de hoy si no hay fecha
            if (!fecha) {
                const today = new Date().toISOString().split('T')[0];
                document.getElementById('fechaPoder').value = today;
            }
        }
        this.updatePreview();
    }
    ...
    checkForDuplicateRUT() {
        const rutMandante = document.getElementById('rutMandante').value;
        const rutMandatario = document.getElementById('rutMandatario').value;
        
        if (rutMandante && rutMandatario && rutMandante === rutMandatario) {
            this.showNotification('El mandante y mandatario no pueden tener el mismo RUT', 'warning');
        }
    }
    ...
    updateDatePreview() {
        const fechaPoder = document.getElementById('fechaPoder').value;
        
        if (fechaPoder && ConfigUtils.validateDate(fechaPoder)) {
            const date = new Date(fechaPoder);
    ...
    updateSignatureNames() {
        const nombreMandante = document.getElementById('nombreMandante').value;
        
        const firmaMandante = document.getElementById('prev_firmaMandante');
    ...
    updateProgress() {
        ...
        if (systemState.firmasDigitales.mandante) bonusScore += 0.5;
        if (systemState.firmasDigitales.mandatario) bonusScore += 0.5;
        
        if (systemState.fotosCarnet.mandante.completed) bonusScore += 0.5;
        if (systemState.fotosCarnet.mandatario.completed) bonusScore += 0.5;
    ...
    async initializeSignatureSystemSafe() {
        ...
        const requiredElements = ['firmaModalOverlay', 'firmaMandante', 'firmaMandatario'];
    ...
    async setupSignatureButtonsSafe() {
        ...
        const btnFirmaMandante = document.getElementById('firmaMandante');
        const btnFirmaMandatario = document.getElementById('firmaMandatario');
        
        if (!btnFirmaMandante || !btnFirmaMandatario) {
            throw new Error('Botones de firma no encontrados');
        }
        
        // Remover listeners existentes
        const clonedBtnMandante = btnFirmaMandante.cloneNode(true);
        const clonedBtnMandatario = btnFirmaMandatario.cloneNode(true);
        
        btnFirmaMandante.parentNode.replaceChild(clonedBtnMandante, btnFirmaMandante);
        btnFirmaMandatario.parentNode.replaceChild(clonedBtnMandatario, btnFirmaMandatario);
        
        // Configurar nuevos listeners
        clonedBtnMandante.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('🖋️ Clic en firma mandante');
            this.openSignatureModalSafe('mandante');
        });
        
        clonedBtnMandatario.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('🖋️ Clic en firma mandatario');
            this.openSignatureModalSafe('mandatario');
        });
    ...
    async initializePhotoSystemSafe() {
        ...
        const requiredElements = ['photoModalOverlay', 'photoMandante', 'photoMandatario'];
        const missingElements = requiredElements.filter(id => !document.getElementById(id));
    ...
    async setupPhotoButtonsSafe() {
        ...
        const btnPhotoMandante = document.getElementById('photoMandante');
        const btnPhotoMandatario = document.getElementById('photoMandatario');
        ...
        // Remover listeners existentes
        const clonedBtnPhotoMandante = btnPhotoMandante.cloneNode(true);
        const clonedBtnPhotoMandatario = btnPhotoMandatario.cloneNode(true);
        
        btnPhotoMandante.parentNode.replaceChild(clonedBtnPhotoMandante, btnPhotoMandante);
        btnPhotoMandatario.parentNode.replaceChild(clonedBtnPhotoMandatario, btnPhotoMandatario);
        
        // Configurar nuevos listeners
        clonedBtnPhotoMandante.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('📷 Clic en foto mandante');
            this.openPhotoModalSafe('mandante');
        });
        
        clonedBtnPhotoMandatario.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('📷 Clic en foto mandatario');
            this.openPhotoModalSafe('mandatario');
        });
    ...
    initializeFormFields() {
        ...
        const fechaPoder = document.getElementById('fechaPoder');
        if (fechaPoder && !fechaPoder.value) {
            const today = new Date().toISOString().split('T')[0];
            fechaPoder.value = today;
        }
            ...
            if (savedData) {
                ...
                Object.keys(data).forEach(field => {
                    if (field === 'firmasDigitales') {
                        systemState.firmasDigitales = data.firmasDigitales || { mandante: false, mandatario: false };
    ...
    clearForm() {
        this.showConfirmDialog(
            ...
            () => {
                ...
                systemState.firmasDigitales = { mandante: false, mandatario: false };
                systemState.fotosCarnet = {
                    mandante: { frente: { captured: false, imageData: null, timestamp: null }, reverso: { captured: false, imageData: null, timestamp: null }, completed: false },
                    mandatario: { frente: { captured: false, imageData: null, timestamp: null }, reverso: { captured: false, imageData: null, timestamp: null }, completed: false }
                };
    ...
    // NUEVO: Manejo de errores de inicialización
    ...
    // NUEVO: Inicialización de respaldo
    ...
    async generatePDF() {
        ...
        try {
            ...
            setTimeout(() => {
                doc.save(fileName);
                this.showNotification('¡PDF de Poder Simple generado exitosamente!', 'success');
            }, 1500);
    ...
    async addPDFContent(doc) {
        ...
        doc.setFontSize(16);
        doc.text('PODER SIMPLE', 105, y, { align: 'center' });
        y += 25;
        ...
        const ciudadPoder = document.getElementById('ciudadPoder').value;
        const fechaPoder = new Date(document.getElementById('fechaPoder').value);
        
        const encabezado = `En ${ciudadPoder} de Chile, a ${fechaPoder.getDate()} de ${this.mesesEspanol[fechaPoder.getMonth()]} de ${fechaPoder.getFullYear()}`;
        ...
        const nombreMandante = document.getElementById('nombreMandante').value;
        const rutMandante = document.getElementById('rutMandante').value;
        const domicilioMandante = document.getElementById('domicilioMandante').value;
        const comunaMandante = document.getElementById('comunaMandante').value;
        const ciudadMandante = document.getElementById('ciudadMandante').value;

        doc.text(`Don (doña) ${nombreMandante}`, margin, y);
        y += 8;
        doc.text(`C.I. Nº ${rutMandante}`, margin, y);
        y += 8;
        doc.text(`Con domicilio en ${domicilioMandante}`, margin, y);
        y += 8;
        doc.text(`Comuna de ${comunaMandante}`, margin, y);
        y += 8;
        doc.text(`De la ciudad de ${ciudadMandante}`, margin, y);
        y += 15;

        doc.setFont('times', 'bold');
        doc.text('Viene a conferir poder a:', margin, y);
        y += 15;

        // Datos del Mandatario
        doc.setFont('times', 'normal');
        const nombreMandatario = document.getElementById('nombreMandatario').value;
        const rutMandatario = document.getElementById('rutMandatario').value;
        const domicilioMandatario = document.getElementById('domicilioMandatario').value;
        const comunaMandatario = document.getElementById('comunaMandatario').value;
        const ciudadMandatario = document.getElementById('ciudadMandatario').value;

        doc.text(`Don (doña) ${nombreMandatario}`, margin, y);
        y += 8;
        doc.text(`C.I. Nº ${rutMandatario}`, margin, y);
        y += 8;
        doc.text(`Con domicilio en ${domicilioMandatario}`, margin, y);
        y += 8;
        doc.text(`Comuna de ${comunaMandatario}`, margin, y);
        y += 8;
        doc.text(`De la ciudad de ${ciudadMandatario}`, margin, y);
        y += 15;

        // Efectos del poder
        doc.setFont('times', 'bold');
        doc.text('Para los siguientes efectos:', margin, y);
        y += 12;

        doc.setFont('times', 'normal');
        const efectosPoder = document.getElementById('efectosPoder').value;
        const splitEfectos = doc.splitTextToSize(efectosPoder, pageWidth - 2 * margin);
        doc.text(splitEfectos, margin, y);
        y += splitEfectos.length * 6 + 15;

        // Cláusula legal
        const clausulaLegal = CONFIG.DOCUMENT.legal_texts.clausula_final;
        const splitClausula = doc.splitTextToSize(clausulaLegal, pageWidth - 2 * margin);
        doc.text(splitClausula, margin, y);
        y += splitClausula.length * 6 + 30;
        ...
        doc.setFont('times', 'bold');
        doc.setFontSize(10);
        const nota = CONFIG.DOCUMENT.legal_texts.nota_laboral;
        const splitNota = doc.splitTextToSize(nota, pageWidth - 2 * margin);
    ...
    addSignatureSection(doc, y) {
        ...
        doc.setFont('times', 'bold');
        doc.setFontSize(11);
        doc.text(document.getElementById('nombreMandante').value, 105, y, { align: 'center' });
        y += 6;
        
        doc.setFont('times', 'normal');
        doc.setFontSize(10);
        doc.text('Firma del Mandante', 105, y, { align: 'center' });
        
        // Agregar firma digital si existe
        if (window.firmaProcessor) {
            try {
                const firmaMandante = window.firmaProcessor.getSignatureForPDF('mandante');
                
                if (firmaMandante) {
                    doc.addImage(firmaMandante, 'PNG', 60, y - 40, 90, 30);
    ...
    generateFileName() {
        ...
        return `Poder_Simple_${nombreMandante}_${timestamp}.pdf`;
    }
    ...
    validateFormWithSignatures() {
        ...
        if (rutMandante && !ConfigUtils.validateRUT(rutMandante)) {
            issues.push('RUT del mandante inválido');
        }
        
        if (rutMandatario && !ConfigUtils.validateRUT(rutMandatario)) {
            issues.push('RUT del mandatario inválido');
        }
        
        if (rutMandante && rutMandatario && rutMandante === rutMandatario) {
            issues.push('El mandante y mandatario no pueden ser la misma persona');
        }
        
        const efectos = document.getElementById('efectosPoder').value;
        if (efectos && !ConfigUtils.validateTextLength(efectos, 10, 2000)) {
            issues.push('Los efectos del poder deben tener entre 10 y 2000 caracteres');
        }
        
        if (!systemState.firmasDigitales.mandante) {
...
// ===== CLASES AUXILIARES (sin cambios pero con verificaciones mejoradas) =====
...
class FirmaDigitalProcessor {
    ...
    applySignature() {
        if (this.currentType && this.processedImageData) {
            ...
            if (window.poderSystem) {
                window.poderSystem.updateProgress();
            }
            
            this.closeModal();
            
            if (window.poderSystem) {
                window.poderSystem.showNotification(`Firma digital del ${this.currentType} aplicada correctamente`, 'success');
            }
        }
    }
    ...
    clearSignatures() {
        this.firmasDigitales.mandante = null;
        this.firmasDigitales.mandatario = null;
        
        systemState.firmasDigitales = { mandante: false, mandatario: false };
        
        ['mandante', 'mandatario'].forEach(type => {
...
class PhotoCarnetDualProcessor {
    ...
    handleCameraError(error) {
        ...
        if (window.poderSystem) {
            window.poderSystem.showNotification(errorMessage + '. ' + detailedInfo, 'error');
        }
    ...
    capturePhoto() {
        ...
        try {
            ...
            if (window.poderSystem) {
                window.poderSystem.showNotification(`Foto del ${this.currentStep} capturada`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al capturar foto:', error);
            if (window.poderSystem) {
                window.poderSystem.showNotification('Error al capturar la foto', 'error');
            }
        }
    ...
    nextStep() {
        if (this.currentStep === 'frente' && this.tempImages.frente) {
            ...
            this.showCamera();
            if (window.poderSystem) {
                window.poderSystem.showNotification('Ahora capture el reverso del carnet', 'info');
            }
        } else if (this.currentStep === 'reverso' && this.tempImages.reverso) {
            this.currentStep = 'completed';
            this.showSummary();
            if (window.poderSystem) {
                window.poderSystem.showNotification('Ambas fotos capturadas', 'success');
            }
        }
    }
    ...
    restartProcess() {
        ...
        this.showCamera();
        if (window.poderSystem) {
            window.poderSystem.showNotification('Proceso reiniciado', 'info');
        }
    }
    ...
    async confirmAllPhotos() {
        if (!this.tempImages.frente || !this.tempImages.reverso || !this.currentType) {
            if (window.poderSystem) {
                window.poderSystem.showNotification('Faltan fotos por capturar', 'error');
            }
            return;
        }
        ...
        try {
            ...
            if (window.poderSystem) {
                window.poderSystem.showNotification(`Carnet completo del ${this.currentType} guardado`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al confirmar fotos:', error);
            if (window.poderSystem) {
                window.poderSystem.showNotification('Error al procesar las fotos', 'error');
            }
        }
    }
...
window.clearForm = function() {
    if (window.poderSystem) {
        window.poderSystem.clearForm();
    }
};

window.generatePDF = function() {
    if (window.poderSystem) {
        window.poderSystem.generatePDF();
    }
};
...
// NUEVO: Función de inicialización robusta
function initializePoderSystem() {
    console.log('🏛️ Iniciando Sistema de Poder Simple...');
    
    // Verificar que jsPDF esté disponible
    if (typeof window.jspdf === 'undefined') {
        console.warn('⚠️ jsPDF no está disponible, esperando...');
        setTimeout(initializePoderSystem, 1000);
        return;
    }
    ...
    try {
        poderSystem = new PoderSimpleSystem();
        window.poderSystem = poderSystem;
        
        console.log('✅ Sistema completamente inicializado');
    } catch (error) {
        console.error('❌ Error crítico en inicialización:', error);
        setTimeout(initializePoderSystem, 2000);
    }
}

// NUEVO: Múltiples puntos de inicialización para asegurar carga
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializePoderSystem);
} else if (document.readyState === 'interactive' || document.readyState === 'complete') {
    // Si ya está cargado, esperar un poco y ejecutar
    setTimeout(initializePoderSystem, 500);
}

// NUEVO: Respaldo adicional
...
console.log('📜 Script principal de Poder Simple cargado - VERSIÓN CORREGIDA');