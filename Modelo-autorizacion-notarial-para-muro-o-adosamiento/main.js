// ========================================
// 📋 SISTEMA PRINCIPAL DE AUTORIZACIÓN NOTARIAL PARA MURO O ADOSAMIENTO
// ========================================
...
const systemState = {
    formFields: [
        ...
        'nombreAutorizante', 'cedulaAutorizante', 'telefonoAutorizante', 
        'calleAutorizante', 'numeroAutorizante', 'villaAutorizante', 'comunaAutorizante',
        'nombreAutorizado', 'cedulaAutorizado', 'calleAutorizado', 'numeroAutorizado',
        'longitudEdificacion', 'alturaMaxima', 'tipoConstruccion',
        'propositoAutorizacion', 'observacionesAdicionales'
    ],
...
class AutorizacionNotarialSystem {
    constructor() {
    ...
    async init() {
        console.log('📋 Inicializando Sistema de Autorización Notarial...');
    ...
    async ensureCriticalElements() {
        const criticalElements = [
            ...
            'firmaAutorizante', 'photoAutorizante',
    ...
    setupSpecialFieldListeners() {
        // Validación Cédula Autorizante
        const cedulaAutorizante = document.getElementById('cedulaAutorizante');
        if (cedulaAutorizante) {
            cedulaAutorizante.addEventListener('input', (e) => {
                this.handleCedulaInput(e, 'cedulaAutorizante');
            });
        }

        // Validación Cédula Autorizado
        const cedulaAutorizado = document.getElementById('cedulaAutorizado');
        if (cedulaAutorizado) {
            cedulaAutorizado.addEventListener('input', (e) => {
                this.handleCedulaInput(e, 'cedulaAutorizado');
            });
        }
        ...
        // Validar longitud de edificación
        const longitudEdificacion = document.getElementById('longitudEdificacion');
        if (longitudEdificacion) {
            longitudEdificacion.addEventListener('input', (e) => {
                const valor = parseFloat(e.target.value);
                if (!isNaN(valor) && ConfigUtils.validateLongitud(valor)) {
                    e.target.classList.add('longitud-valid');
                    e.target.classList.remove('longitud-invalid');
                } else if (e.target.value !== '') {
                    e.target.classList.add('longitud-invalid');
                    e.target.classList.remove('longitud-valid');
                } else {
                    e.target.classList.remove('longitud-valid', 'longitud-invalid');
                }
                this.updatePreview();
            });
        }

        // Validar altura de edificación
        const alturaMaxima = document.getElementById('alturaMaxima');
        if (alturaMaxima) {
            alturaMaxima.addEventListener('input', (e) => {
                const valor = parseFloat(e.target.value);
                if (!isNaN(valor) && ConfigUtils.validateAltura(valor)) {
                    e.target.classList.add('altura-valid');
                    e.target.classList.remove('altura-invalid');
                } else if (e.target.value !== '') {
                    e.target.classList.add('altura-invalid');
                    e.target.classList.remove('altura-valid');
                } else {
                    e.target.classList.remove('altura-valid', 'altura-invalid');
                }
                this.updatePreview();
            });
        }
        ...
        const numeroAutorizante = document.getElementById('numeroAutorizante');
        if (numeroAutorizante) {
            numeroAutorizante.addEventListener('input', (e) => {
        ...
        const numeroAutorizado = document.getElementById('numeroAutorizado');
        if (numeroAutorizado) {
            numeroAutorizado.addEventListener('input', (e) => {
                if (ConfigUtils.validateNumeroAddress(e.target.value)) {
                    e.target.classList.add('numero-valid');
                    e.target.classList.remove('numero-invalid');
                } else if (e.target.value !== '') {
    ...
    updateDuplicateReferences() {
        ...
        const cedulaAutorizante = document.getElementById('cedulaAutorizante').value;
        
        const prev_nombreAutorizante2 = document.getElementById('prev_nombreAutorizante2');
        const prev_cedulaAutorizante2 = document.getElementById('prev_cedulaAutorizante2');
        
        if (prev_cedulaAutorizante2) {
            prev_cedulaAutorizante2.textContent = cedulaAutorizante || 'XX.XXX.XXX-X';
        }
    }
    ...
    getPlaceholderText(field) {
        const placeholders = {
            ...
            'nombreAutorizante': '_________________________',
            'nombreAutorizado': '_________________________',
            'cedulaAutorizante': 'XX.XXX.XXX-X',
            'cedulaAutorizado': 'XX.XXX.XXX-X',
            'calleAutorizante': '_________________________',
            'calleAutorizado': '_________________________',
            'numeroAutorizante': '____',
            'numeroAutorizado': '____',
            'villaAutorizante': '_________________________',
            'comunaAutorizante': '___________',
            'longitudEdificacion': '___',
            'alturaMaxima': '3,50',
            'tipoConstruccion': 'adosada al deslinde común o con uso de medianería',
            'propositoAutorizacion': 'certificar ante la Dirección de Obras Municipales',
            'observacionesAdicionales': 'Sin observaciones adicionales',
            'telefonoAutorizante': '___________'
        };
    ...
    openSignatureModalSafe(type) {
        ...
        let nombreField = document.getElementById('nombreAutorizante');
    ...
    async initializePhotoSystemSafe() {
        ...
        const requiredElements = ['photoModalOverlay', 'photoAutorizante'];
        const missingElements = requiredElements.filter(id => !document.getElementById(id));
            ...
            if (savedData) {
                ...
                Object.keys(data).forEach(field => {
                    if (field === 'firmasDigitales') {
                        systemState.firmasDigitales = data.firmasDigitales || { autorizante: false };
        ...
        this.showConfirmDialog(
            ...
            () => {
                systemState.formFields.forEach(field => {
                    ...
                    if (element) {
                        element.value = '';
                        element.classList.remove('cedula-valid', 'cedula-invalid', 'longitud-valid', 'longitud-invalid', 'altura-valid', 'altura-invalid', 'numero-valid', 'numero-invalid');
                    }
                });
                ...
                const alturaMaxima = document.getElementById('alturaMaxima');
                if (alturaMaxima) {
                    alturaMaxima.value = '3.50';
                }
                
                systemState.firmasDigitales = { autorizante: false };
    ...
    async addPDFContent(doc) {
        ...
        doc.setFontSize(18);
        doc.text('AUTORIZACIÓN NOTARIAL', 105, y, { align: 'center' });
        y += 10;
        doc.setFontSize(14);
        doc.text('Para Construcción de Edificación Adosada', 105, y, { align: 'center' });
        y += 40;
        ...
        const nombreAutorizante = document.getElementById('nombreAutorizante').value;
        const cedulaAutorizante = document.getElementById('cedulaAutorizante').value;
        const calleAutorizante = document.getElementById('calleAutorizante').value;
        const numeroAutorizante = document.getElementById('numeroAutorizante').value;
        const villaAutorizante = document.getElementById('villaAutorizante').value;
        const comunaAutorizante = document.getElementById('comunaAutorizante').value;
        const nombreAutorizado = document.getElementById('nombreAutorizado').value;
        const cedulaAutorizado = document.getElementById('cedulaAutorizado').value;
        const calleAutorizado = document.getElementById('calleAutorizado').value;
        const numeroAutorizado = document.getElementById('numeroAutorizado').value;
        const longitudEdificacion = document.getElementById('longitudEdificacion').value;
        ...
        // Párrafo principal de autorización
        const parrafoAutorizacion = `Por el presente instrumento, yo ${nombreAutorizante}, Cédula de Identidad N° ${cedulaAutorizante}, propietario (a) del inmueble ubicado en ${calleAutorizante} N° ${numeroAutorizante} de la ${villaAutorizante}, comuna de ${comunaAutorizante}, autorizo a don (ña) ${nombreAutorizado} Cédula de Identidad N° ${cedulaAutorizado} propietario (a) del inmueble ubicado en el predio vecino en ${calleAutorizado} N° ${numeroAutorizado} para que Construya una edificación de ${longitudEdificacion} metros de longitud, ${tipoConstruccion}, y una altura máxima de ${alturaMaxima} metros de acuerdo al art. 2.6.2. de la O.G.U.y C.`;
        
        doc.text(parrafoAutorizacion, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, parrafoAutorizacion, pageWidth - 2 * margin) + 20;

        // Propósito de la autorización
        const propositoAutorizacion = document.getElementById('propositoAutorizacion').value;
        const parrafoPurpose = `La presente autorización es para ${propositoAutorizacion} de mi conformidad con la construcción que se proyecta edificar y regularizar.`;
        
        doc.text(parrafoPurpose, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, parrafoPurpose, pageWidth - 2 * margin) + 20;
        ...
        if (observacionesAdicionales && observacionesAdicionales.trim()) {
            const observacionesTexto = `Observaciones: ${observacionesAdicionales}`;
            doc.text(observacionesTexto, margin, y, {maxWidth: pageWidth - 2 * margin});
            y += this.calculateTextHeight(doc, observacionesTexto, pageWidth - 2 * margin) + 30;
        }
        ...
        // Fecha y ciudad
        const fechaDocumento = document.getElementById('fechaDocumento').value;
        const ciudadDocumento = document.getElementById('ciudadDocumento').value || 'Santiago';

        let fechaTexto = '__, __ de ________ de 20__';
        if (fechaDocumento && ConfigUtils.validateDate(fechaDocumento)) {
            const date = new Date(fechaDocumento);
            fechaTexto = `${ciudadDocumento}, ${date.getDate()} de ${this.mesesEspanol[date.getMonth()]} de ${date.getFullYear()}`;
        }

        doc.text(fechaTexto, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += 50;
    ...
    addSignatureSection(doc, y) {
        ...
        doc.setFontSize(11);
        const nombreAutorizante = document.getElementById('nombreAutorizante').value;
        doc.text(nombreAutorizante, centerX, y, { align: 'center' });
        y += 6;
        ...
        doc.setFont('times', 'normal');
        doc.setFontSize(9);
        doc.text(`C.I. N° ${cedulaAutorizante}`, centerX, y, { align: 'center' });
        ...
        if (window.firmaProcessor) {
            try {
                ...
                if (firmaAutorizante) {
                    doc.addImage(firmaAutorizante, 'PNG', centerX - 40, y - 30, 80, 20);
    ...
    generateFileName() {
        ...
        return `Autorizacion_Notarial_${nombreAutorizante}_${nombreAutorizado}_${timestamp}.pdf`;
    }
    ...
    validateFormWithSignatures() {
        ...
        const requiredFields = [
            'ciudadDocumento', 'fechaDocumento',
            'nombreAutorizante', 'cedulaAutorizante', 'calleAutorizante', 'numeroAutorizante', 'comunaAutorizante',
            'nombreAutorizado', 'cedulaAutorizado', 'calleAutorizado', 'numeroAutorizado',
            'longitudEdificacion', 'alturaMaxima', 'tipoConstruccion'
        ];
        ...
        if (cedulaAutorizante && !ConfigUtils.validateCedula(cedulaAutorizante)) {
            issues.push('Cédula del autorizante inválida');
        }
        
        if (cedulaAutorizado && !ConfigUtils.validateCedula(cedulaAutorizado)) {
            issues.push('Cédula del autorizado inválida');
        }
        ...
        // Validar longitud y altura
        const longitudEdificacion = document.getElementById('longitudEdificacion').value;
        if (longitudEdificacion && !ConfigUtils.validateLongitud(longitudEdificacion)) {
            issues.push('Longitud de edificación inválida');
        }
        
        const alturaMaxima = document.getElementById('alturaMaxima').value;
        if (alturaMaxima && !ConfigUtils.validateAltura(alturaMaxima)) {
            issues.push('Altura máxima inválida');
        }
...
class FirmaDigitalProcessor {
    ...
    applySignature() {
        if (this.currentType && this.processedImageData) {
            ...
            if (window.autorizacionNotarialSystem) {
                window.autorizacionNotarialSystem.updateProgress();
            }
            
            this.closeModal();
            
            if (window.autorizacionNotarialSystem) {
                window.autorizacionNotarialSystem.showNotification(`Firma digital del ${this.currentType} aplicada correctamente`, 'success');
            }
        }
    }
    ...
    clearSignatures() {
        ...
        const buttonId = 'firmaAutorizante';
        const button = document.getElementById(buttonId);
        if (button) {
        ...
        const placeholderId = 'signaturePlaceholderAutorizante';
        const previewId = 'signaturePreviewAutorizante';
...
class PhotoCarnetDualProcessor {
    ...
    handleCameraError(error) {
        ...
        if (window.autorizacionNotarialSystem) {
            window.autorizacionNotarialSystem.showNotification(errorMessage, 'error');
        }
    }
    ...
    capturePhoto() {
        ...
        try {
            ...
            if (window.autorizacionNotarialSystem) {
                window.autorizacionNotarialSystem.showNotification(`Foto del ${this.currentStep} capturada`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al capturar foto:', error);
            if (window.autorizacionNotarialSystem) {
                window.autorizacionNotarialSystem.showNotification('Error al capturar la foto', 'error');
            }
        }
    ...
    nextStep() {
        if (this.currentStep === 'frente' && this.tempImages.frente) {
            ...
            this.showCamera();
            if (window.autorizacionNotarialSystem) {
                window.autorizacionNotarialSystem.showNotification('Ahora capture el reverso del carnet', 'info');
            }
        } else if (this.currentStep === 'reverso' && this.tempImages.reverso) {
            this.currentStep = 'completed';
            this.showSummary();
            if (window.autorizacionNotarialSystem) {
                window.autorizacionNotarialSystem.showNotification('Ambas fotos capturadas', 'success');
            }
        }
    }
    ...
    restartProcess() {
        ...
        this.showCamera();
        if (window.autorizacionNotarialSystem) {
            window.autorizacionNotarialSystem.showNotification('Proceso reiniciado', 'info');
        }
    }
    ...
    async confirmAllPhotos() {
        if (!this.tempImages.frente || !this.tempImages.reverso || !this.currentType) {
            if (window.autorizacionNotarialSystem) {
                window.autorizacionNotarialSystem.showNotification('Faltan fotos por capturar', 'error');
            }
            return;
        }
        ...
        try {
            ...
            if (window.autorizacionNotarialSystem) {
                window.autorizacionNotarialSystem.showNotification(`Carnet completo del ${this.currentType} guardado`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al confirmar fotos:', error);
            if (window.autorizacionNotarialSystem) {
                window.autorizacionNotarialSystem.showNotification('Error al procesar las fotos', 'error');
            }
        }
    }
...
window.clearForm = function() {
    if (window.autorizacionNotarialSystem) {
        window.autorizacionNotarialSystem.clearForm();
    }
};

window.generatePDF = function() {
    if (window.autorizacionNotarialSystem) {
        window.autorizacionNotarialSystem.generatePDF();
    }
};

// ===== INICIALIZACIÓN DEL SISTEMA =====
let autorizacionNotarialSystem;

function initializeAutorizacionNotarialSystem() {
    console.log('📋 Iniciando Sistema de Autorización Notarial...');
    
    // Verificar que jsPDF esté disponible
    if (typeof window.jspdf === 'undefined') {
        console.warn('⚠️ jsPDF no está disponible, esperando...');
        setTimeout(initializeAutorizacionNotarialSystem, 1000);
        return;
    }
    ...
    try {
        autorizacionNotarialSystem = new AutorizacionNotarialSystem();
        window.autorizacionNotarialSystem = autorizacionNotarialSystem;
        
        console.log('✅ Sistema completamente inicializado');
    } catch (error) {
        console.error('❌ Error crítico en inicialización:', error);
        setTimeout(initializeAutorizacionNotarialSystem, 2000);
    }
}

// ===== MÚLTIPLES PUNTOS DE INICIALIZACIÓN =====
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeAutorizacionNotarialSystem);
} else if (document.readyState === 'interactive' || document.readyState === 'complete') {
    setTimeout(initializeAutorizacionNotarialSystem, 500);
}
...
window.addEventListener('error', (event) => {
    ...
    if (window.autorizacionNotarialSystem) {
        window.autorizacionNotarialSystem.showNotification(
...
// ===== LOG FINAL =====
console.log('📜 Sistema de Autorización Notarial - Script principal cargado correctamente');