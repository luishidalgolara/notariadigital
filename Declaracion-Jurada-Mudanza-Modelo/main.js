// ========================================
// 📋 SISTEMA PRINCIPAL DE DECLARACIÓN JURADA SALVOCONDUCTO MUDANZA
// ========================================
...
const systemState = {
    formFields: [
        ...
        'nombreDeclarante', 'cedulaDeclarante', 'comunaDeclarante', 'domicilioDeclarante',
        'fechaMudanza', 'horaMudanza', 'direccionOrigen', 'comunaOrigen', 'regionOrigen',
        'direccionDestino', 'comunaDestino', 'regionDestino',
        'patenteVehiculo', 'tipoVehiculo', 'descripcionVehiculo',
        'nombreAcompanante1', 'rutAcompanante1', 'telefonoAcompanante1',
        'nombreAcompanante2', 'rutAcompanante2', 'telefonoAcompanante2',
        'motivoMudanza', 'descripcionEnseres', 'observacionesEspeciales',
...
class DeclaracionJuradaSystem {
    ...
    async init() {
        console.log('📋 Inicializando Sistema de Declaración Jurada Salvoconducto Mudanza...');
    ...
    async ensureCriticalElements() {
        const criticalElements = [
            ...
            'firmaDeclarante', 'firmaAcompanante', 'photoDeclarante', 'photoAcompanante',
    ...
    setupSpecialFieldListeners() {
        // Validación RUTs/Cédulas
        const rutFields = ['cedulaDeclarante', 'rutAcompanante1', 'rutAcompanante2'];
        ...
        if (patenteVehiculo) {
            patenteVehiculo.addEventListener('input', (e) => {
                ...
                const formatted = ConfigUtils.formatPatente(e.target.value);
                if (formatted !== e.target.value) {
                    e.target.value = formatted;
                }
                
                if (e.target.value && !ConfigUtils.validatePatente(e.target.value)) {
                    this.showNotification('Formato de patente inválido (ej: ABCD-12)', 'warning');
                }
                this.updatePreview();
            });
        }
        ...
        // Validar teléfonos
        const phoneFields = ['telefonoAcompanante1', 'telefonoAcompanante2', 'telefonoEmergencia'];
        ...
        // Validar fecha de mudanza
        const fechaMudanza = document.getElementById('fechaMudanza');
        const fechaDocumento2 = document.getElementById('fechaDocumento');
        if (fechaMudanza) {
            fechaMudanza.addEventListener('change', (e) => {
                if (!ConfigUtils.validateFechaMudanza(e.target.value)) {
                    this.showNotification('La fecha de mudanza debe estar entre 30 días atrás y 30 días adelante', 'warning');
                }
                
                if (fechaDocumento2 && fechaDocumento2.value) {
                    if (!ConfigUtils.validateFechasMudanza(fechaDocumento2.value, e.target.value)) {
                        this.showNotification('La fecha de mudanza no puede ser anterior a la fecha del documento', 'warning');
                    }
                }
        ...
        // Mostrar/ocultar observaciones especiales en vista previa
        ...
        if (observacionesEspeciales) {
            observacionesEspeciales.addEventListener('input', () => {
                ...
                if (container) {
                    container.style.display = observacionesEspeciales.value.trim() ? 'block' : 'none';
                }
                this.updatePreview();
            });
        }
    ...
    updatePreview() {
        systemState.formFields.forEach(field => {
            ...
            if (element && preview) {
                ...
                if (field === 'motivoMudanza') {
                    value = ConfigUtils.formatMotivoMudanza(value);
                } else if (field === 'tipoVehiculo') {
                    value = ConfigUtils.formatTipoVehiculo(value);
                }
    ...
    updateDatePreview() {
        ...
        // Actualizar fecha de mudanza formateada
        const fechaMudanza = document.getElementById('fechaMudanza').value;
        const prev_fechaMudanzaFormateada = document.getElementById('prev_fechaMudanzaFormateada');
        if (prev_fechaMudanzaFormateada) {
            if (fechaMudanza && ConfigUtils.validateDate(fechaMudanza)) {
                prev_fechaMudanzaFormateada.textContent = ConfigUtils.formatDateToSpanish(fechaMudanza);
            } else {
                prev_fechaMudanzaFormateada.textContent = '__ de ________ de 20__';
            }
        }
    }
    ...
    updateSignatureNames() {
        ...
        const nombreAcompanante1 = document.getElementById('nombreAcompanante1').value;
        
        const prev_firmaDeclarante = document.getElementById('prev_firmaDeclarante');
        const prev_firmaAcompanante = document.getElementById('prev_firmaAcompanante');
        
        if (prev_firmaDeclarante) prev_firmaDeclarante.textContent = nombreDeclarante || '_______________';
        if (prev_firmaAcompanante) prev_firmaAcompanante.textContent = nombreAcompanante1 || '_______________';
    }

    updateSpecialFields() {
        // Actualizar hora de mudanza
        const horaMudanza = document.getElementById('horaMudanza').value;
        const prev_horaMudanza = document.getElementById('prev_horaMudanza');
        if (prev_horaMudanza) {
            prev_horaMudanza.textContent = horaMudanza || '_____';
        }

        // Formatear patente en mayúsculas
        const patenteVehiculo = document.getElementById('patenteVehiculo').value;
        const prev_patenteVehiculo = document.getElementById('prev_patenteVehiculo');
        if (prev_patenteVehiculo) {
            prev_patenteVehiculo.textContent = patenteVehiculo.toUpperCase() || '________';
        }
    }
    ...
    getPlaceholderText(field) {
        const placeholders = {
            ...
            'domicilioDeclarante': '________________',
            'direccionOrigen': '________________',
            'comunaOrigen': '________',
            'regionOrigen': 'Región Metropolitana',
            'direccionDestino': '________________',
            'comunaDestino': '________',
            'regionDestino': 'Región Metropolitana',
            'patenteVehiculo': '________',
            'tipoVehiculo': '________',
            'descripcionVehiculo': '________________',
            'nombreAcompanante1': '________________',
            'rutAcompanante1': '___________',
            'telefonoAcompanante1': '___________',
            'nombreAcompanante2': '________________',
            'rutAcompanante2': '___________',
            'telefonoAcompanante2': '___________',
            'motivoMudanza': '________________',
            'descripcionEnseres': '________________',
    ...
    updateProgress() {
        ...
        if (systemState.firmasDigitales.declarante) bonusScore += 0.25;
        if (systemState.firmasDigitales.acompanante) bonusScore += 0.25;
        if (systemState.fotosCarnet.declarante.completed) bonusScore += 0.25;
        if (systemState.fotosCarnet.acompanante.completed) bonusScore += 0.25;
    ...
    async initializeSignatureSystemSafe() {
        ...
        const requiredElements = ['firmaModalOverlay', 'firmaDeclarante', 'firmaAcompanante'];
    ...
    async setupSignatureButtonsSafe() {
        ...
        if (!btnFirmaDeclarante || !btnFirmaAcompanante) {
    ...
    openSignatureModalSafe(type) {
        ...
        let nombreField;
        if (type === 'declarante') {
            nombreField = document.getElementById('nombreDeclarante');
        } else if (type === 'acompanante') {
            nombreField = document.getElementById('nombreAcompanante1');
        }
    ...
    async initializePhotoSystemSafe() {
        ...
        const requiredElements = ['photoModalOverlay', 'photoDeclarante', 'photoAcompanante'];
        const missingElements = requiredElements.filter(id => !document.getElementById(id));
    ...
    async setupPhotoButtonsSafe() {
        ...
        if (!btnPhotoDeclarante || !btnPhotoAcompanante) {
            ...
            if (savedData) {
                ...
                Object.keys(data).forEach(field => {
                    if (field === 'firmasDigitales') {
                        systemState.firmasDigitales = data.firmasDigitales || { declarante: false, acompanante: false };
    ...
    initializeFormFields() {
        ...
        if (regionOrigen && !regionOrigen.value) {
            regionOrigen.value = 'Región Metropolitana';
        }
        
        const regionDestino = document.getElementById('regionDestino');
        if (regionDestino && !regionDestino.value) {
            regionDestino.value = 'Región Metropolitana';
        }
    ...
    clearForm() {
        this.showConfirmDialog(
            ...
            () => {
                ...
                const regionOrigen = document.getElementById('regionOrigen');
                if (regionOrigen) regionOrigen.value = 'Región Metropolitana';
                
                const regionDestino = document.getElementById('regionDestino');
                if (regionDestino) regionDestino.value = 'Región Metropolitana';
                
                systemState.firmasDigitales = { declarante: false, acompanante: false };
                systemState.fotosCarnet = {
                    declarante: { frente: { captured: false, imageData: null, timestamp: null }, reverso: { captured: false, imageData: null, timestamp: null }, completed: false },
    ...
    async addPDFContent(doc) {
        ...
        doc.setFontSize(18);
        doc.text('DECLARACIÓN JURADA / SALVOCONDUCTO', 105, y, { align: 'center' });
        y += 10;
        
        doc.setFontSize(14);
        doc.text('Mudanza de Enseres', 105, y, { align: 'center' });
        y += 30;
        ...
        const nombreDeclarante = document.getElementById('nombreDeclarante').value;
        const cedulaDeclarante = document.getElementById('cedulaDeclarante').value;
        const domicilioDeclarante = document.getElementById('domicilioDeclarante').value;
        const comunaDeclarante = document.getElementById('comunaDeclarante').value;
        ...
        const encabezado = `En ${ciudadDocumento}, Chile ${fechaTexto}.`;
        ...
        const declarante = `Yo, ${nombreDeclarante}, CÉDULA DE IDENTIDAD N° ${cedulaDeclarante}`;
        doc.text(declarante, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, declarante, pageWidth - 2 * margin) + 10;

        const domicilio = `Domiciliado en ${domicilioDeclarante} comuna de ${comunaDeclarante}.`;
        doc.text(domicilio, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, domicilio, pageWidth - 2 * margin) + 15;

        // Expongo
        doc.setFont('times', 'bold');
        doc.text('Expongo', margin, y);
        y += 15;

        doc.text('DECLARO BAJO JURAMENTO QUE:', margin, y);
        y += 20;
        ...
        const comunaOrigen = document.getElementById('comunaOrigen').value;
        const regionOrigen = document.getElementById('regionOrigen').value;
        const direccionDestino = document.getElementById('direccionDestino').value;
        const comunaDestino = document.getElementById('comunaDestino').value;
        const regionDestino = document.getElementById('regionDestino').value;

        let fechaMudanzaTexto = '__ de ________ de 20__';
        if (fechaMudanza && ConfigUtils.validateDate(fechaMudanza)) {
            fechaMudanzaTexto = ConfigUtils.formatDateToSpanish(fechaMudanza);
        }

        const declaracionMudanza = `El día ${fechaMudanzaTexto} a las ${horaMudanza || '_____'} horas realizaré la mudanza de mis enseres, desde el domicilio ubicado en ${direccionOrigen} comuna de ${comunaOrigen}, ${regionOrigen} hasta ${direccionDestino} comuna de ${comunaDestino}, ${regionDestino}.`;
        
        doc.text(declaracionMudanza, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, declaracionMudanza, pageWidth - 2 * margin) + 15;

        const clausulaLegal = `Declaro además que no tengo impedimento legal, judicial ni contractual, para efectuar dicha mudanza.`;
        doc.text(clausulaLegal, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, clausulaLegal, pageWidth - 2 * margin) + 15;

        // Motivo y enseres
        const motivoMudanza = document.getElementById('motivoMudanza').value;
        const descripcionEnseres = document.getElementById('descripcionEnseres').value;

        if (motivoMudanza) {
            const motivoTexto = `Motivo de la mudanza: ${ConfigUtils.formatMotivoMudanza(motivoMudanza)}.`;
            doc.setFont('times', 'bold');
            doc.text('Motivo de la mudanza: ', margin, y);
            doc.setFont('times', 'normal');
            doc.text(ConfigUtils.formatMotivoMudanza(motivoMudanza), margin + 45, y);
            y += 15;
        }

        if (descripcionEnseres) {
            doc.setFont('times', 'bold');
            doc.text('Descripción de enseres a transportar:', margin, y);
            y += 10;
            doc.setFont('times', 'normal');
            doc.text(descripcionEnseres, margin, y, {maxWidth: pageWidth - 2 * margin});
            y += this.calculateTextHeight(doc, descripcionEnseres, pageWidth - 2 * margin) + 15;
        }

        // Acompañantes
        const nombreAcompanante1 = document.getElementById('nombreAcompanante1').value;
        const rutAcompanante1 = document.getElementById('rutAcompanante1').value;
        const telefonoAcompanante1 = document.getElementById('telefonoAcompanante1').value;
        const nombreAcompanante2 = document.getElementById('nombreAcompanante2').value;
        const rutAcompanante2 = document.getElementById('rutAcompanante2').value;
        const telefonoAcompanante2 = document.getElementById('telefonoAcompanante2').value;

        if (nombreAcompanante1) {
            doc.setFont('times', 'bold');
            doc.text('Además, entrego información de las personas que me acompañarán a efectuar la mudanza:', margin, y);
            y += 15;

            doc.setFont('times', 'normal');
            doc.text(`Acompañante 1: ${nombreAcompanante1}`, margin, y);
            y += 10;
            doc.text(`RUT: ${rutAcompanante1} - Teléfono: ${telefonoAcompanante1}`, margin, y);
            y += 15;

            if (nombreAcompanante2) {
                doc.text(`Acompañante 2: ${nombreAcompanante2}`, margin, y);
                y += 10;
                doc.text(`RUT: ${rutAcompanante2} - Teléfono: ${telefonoAcompanante2}`, margin, y);
                y += 15;
            }
        }
        ...
        const descripcionVehiculo = document.getElementById('descripcionVehiculo').value;

        if (patenteVehiculo) {
            doc.setFont('times', 'bold');
            doc.text('Vehículo utilizado para la mudanza:', margin, y);
            y += 10;
            
            doc.setFont('times', 'normal');
            doc.text(`Patente: ${patenteVehiculo.toUpperCase()}`, margin, y);
            y += 10;
            if (tipoVehiculo || descripcionVehiculo) {
                doc.text(`Tipo: ${ConfigUtils.formatTipoVehiculo(tipoVehiculo)} - ${descripcionVehiculo}`, margin, y);
                y += 15;
            }
        }
        ...
        if (nombreContactoEmergencia) {
            doc.setFont('times', 'bold');
            doc.text('Contacto de emergencia: ', margin, y);
            doc.setFont('times', 'normal');
            doc.text(`${nombreContactoEmergencia} (${relacionEmergencia}) - Tel: ${telefonoEmergencia}`, margin + 45, y);
            y += 15;
        }
        ...
        if (observacionesEspeciales.trim()) {
            ...
            y += this.calculateTextHeight(doc, observacionesEspeciales, pageWidth - 2 * margin) + 15;
        }
    ...
    addSignatureSection(doc, y) {
        ...
        const leftX = 60;  // Posición para declarante
        const rightX = 150; // Posición para acompañantes
        ...
        doc.text(nombreDeclarante, leftX, y, { align: 'center' });
        doc.text(nombreAcompanante1 || 'Acompañante', rightX, y, { align: 'center' });
        y += 6;
        
        // Roles
        doc.setFont('times', 'normal');
        doc.setFontSize(10);
        doc.text('DECLARANTE', leftX, y, { align: 'center' });
        doc.text('ACOMPAÑANTES', rightX, y, { align: 'center' });
        y += 8;
        ...
        doc.setFont('times', 'normal');
        doc.setFontSize(9);
        doc.text(`C.I. N° ${cedulaDeclarante}`, leftX, y, { align: 'center' });
        doc.text('Representante del grupo', rightX, y, { align: 'center' });
        ...
        if (window.firmaProcessor) {
            try {
                ...
                if (firmaDeclarante) {
                    doc.addImage(firmaDeclarante, 'PNG', leftX - 25, y - 30, 50, 20);
                    console.log('✅ Firma del declarante agregada al PDF');
                }
                
                if (firmaAcompanante) {
                    doc.addImage(firmaAcompanante, 'PNG', rightX - 25, y - 30, 50, 20);
                    console.log('✅ Firma de acompañantes agregada al PDF');
                }
                
                if (firmaDeclarante || firmaAcompanante) {
    ...
    generateFileName() {
        ...
        return `Declaracion_Jurada_Mudanza_${nombreDeclarante}_${timestamp}.pdf`;
    }
    ...
    validateFormWithSignatures() {
        ...
        const requiredFields = [
            'ciudadDocumento', 'fechaDocumento',
            'nombreDeclarante', 'cedulaDeclarante', 'comunaDeclarante', 'domicilioDeclarante',
            'fechaMudanza', 'direccionOrigen', 'comunaOrigen',
            'direccionDestino', 'comunaDestino'
        ];
        ...
        if (rutAcompanante1 && !ConfigUtils.validateRUT(rutAcompanante1)) {
            issues.push('RUT del acompañante 1 inválido');
        }
        
        if (rutAcompanante2 && !ConfigUtils.validateRUT(rutAcompanante2)) {
            issues.push('RUT del acompañante 2 inválido');
        }
        ...
        const fechaMudanza = document.getElementById('fechaMudanza').value;
        if (fechaMudanza && !ConfigUtils.validateFechaMudanza(fechaMudanza)) {
            issues.push('Fecha de mudanza fuera del rango permitido');
        }
        
        if (fechaDocumento && fechaMudanza && !ConfigUtils.validateFechasMudanza(fechaDocumento, fechaMudanza)) {
            issues.push('La fecha de mudanza no puede ser anterior al documento');
        }
        ...
        // Validar teléfonos
        const telefonos = ['telefonoAcompanante1', 'telefonoAcompanante2', 'telefonoEmergencia'];
        telefonos.forEach(campo => {
            const elemento = document.getElementById(campo);
            if (elemento && elemento.value && !ConfigUtils.validatePhone(elemento.value)) {
                issues.push(`Teléfono inválido en ${campo}`);
            }
        });
...
class FirmaDigitalProcessor {
    ...
    clearSignatures() {
        ...
        const buttonIds = ['firmaDeclarante', 'firmaAcompanante'];
        ...
        const placeholderIds = ['signaturePlaceholderDeclarante', 'signaturePlaceholderAcompanante'];
        const previewIds = ['signaturePreviewDeclarante', 'signaturePreviewAcompanante'];
...
class PhotoCarnetDualProcessor {
    ...
    clearAllPhotos() {
        const types = ['declarante', 'acompanante'];
        types.forEach(type => {
...
function initializeDeclaracionJuradaSystem() {
    console.log('📋 Iniciando Sistema de Declaración Jurada Salvoconducto Mudanza...');
...
// ===== LOG FINAL =====
console.log('📜 Sistema de Declaración Jurada Salvoconducto Mudanza - Script principal cargado correctamente');