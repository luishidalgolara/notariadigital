// ========================================
// 📋 SISTEMA PRINCIPAL DE SALVOCONDUCTO DE MUDANZA
// ========================================
...
const systemState = {
    formFields: [
        ...
        'direccionInmueble', 'comunaInmueble', 'ciudadInmueble',
        'nombreArrendatario', 'rutArrendatario',
...
class SalvoconductoSystem {
    ...
    async init() {
        console.log('📋 Inicializando Sistema de Salvoconducto de Mudanza...');
    ...
    async ensureCriticalElements() {
        const criticalElements = [
            ...
            'firmaPropietario', 'firmaArrendatario',
            'photoPropietario', 'photoArrendatario',
    ...
    setupSpecialFieldListeners() {
        // Validación RUT Propietario
        const rutPropietarioField = document.getElementById('rutPropietario');
        if (rutPropietarioField) {
            rutPropietarioField.addEventListener('input', (e) => {
                this.handleRutInput(e, 'rutPropietario');
            });
        }

        // Validación RUT Arrendatario
        const rutArrendatarioField = document.getElementById('rutArrendatario');
        if (rutArrendatarioField) {
            rutArrendatarioField.addEventListener('input', (e) => {
    ...
    updateDuplicateReferences() {
        ...
        const rutPropietario = document.getElementById('rutPropietario').value;
        const prev_rutPropietario2 = document.getElementById('prev_rutPropietario2');
        if (prev_rutPropietario2) prev_rutPropietario2.textContent = rutPropietario || '___________';

        const rutArrendatario = document.getElementById('rutArrendatario').value;
        const prev_rutArrendatario2 = document.getElementById('prev_rutArrendatario2');
        if (prev_rutArrendatario2) prev_rutArrendatario2.textContent = rutArrendatario || '___________';
    }
    ...
    updateSignatureNames() {
        ...
        const prev_firmaPropietario = document.getElementById('prev_firmaPropietario');
        if (prev_firmaPropietario) prev_firmaPropietario.textContent = nombrePropietario || '_______________';

        const nombreArrendatario = document.getElementById('nombreArrendatario').value;
        const prev_firmaArrendatario = document.getElementById('prev_firmaArrendatario');
        if (prev_firmaArrendatario) prev_firmaArrendatario.textContent = nombreArrendatario || '_______________';
    }

    getPlaceholderText(field) {
        const placeholders = {
            'ciudadDocumento': 'Santiago',
            'nombrePropietario': '________________',
            'rutPropietario': '___________',
            'direccionInmueble': '________________',
            'comunaInmueble': '________',
            'ciudadInmueble': '________',
            'nombreArrendatario': '________________',
    ...
    updateProgress() {
        const requiredFields = [
            'ciudadDocumento', 'fechaDocumento', 'nombrePropietario', 'rutPropietario',
            'direccionInmueble', 'comunaInmueble', 'ciudadInmueble',
            'nombreArrendatario', 'rutArrendatario'
        ];
        ...
        if (systemState.firmasDigitales.propietario) bonusScore += 0.5;
        if (systemState.firmasDigitales.arrendatario) bonusScore += 0.5;
        if (systemState.fotosCarnet.propietario.completed) bonusScore += 0.25;
    ...
    async initializeSignatureSystemSafe() {
        ...
        const requiredElements = ['firmaModalOverlay', 'firmaPropietario', 'firmaArrendatario'];
    ...
    async setupSignatureButtonsSafe() {
        ...
        const btnFirmaPropietario = document.getElementById('firmaPropietario');
        const btnFirmaArrendatario = document.getElementById('firmaArrendatario');
        
        if (!btnFirmaPropietario || !btnFirmaArrendatario) {
            throw new Error('Botones de firma no encontrados');
        }
        
        btnFirmaPropietario.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openSignatureModalSafe('propietario');
        });

        btnFirmaArrendatario.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openSignatureModalSafe('arrendatario');
        });
    ...
    async initializePhotoSystemSafe() {
        ...
        const requiredElements = ['photoModalOverlay', 'photoPropietario', 'photoArrendatario'];
        const missingElements = requiredElements.filter(id => !document.getElementById(id));
    ...
    async setupPhotoButtonsSafe() {
        ...
        const btnPhotoPropietario = document.getElementById('photoPropietario');
        const btnPhotoArrendatario = document.getElementById('photoArrendatario');
        
        if (!btnPhotoPropietario || !btnPhotoArrendatario) {
            throw new Error('Botones de foto no encontrados');
        }
        
        btnPhotoPropietario.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openPhotoModalSafe('propietario');
        });

        btnPhotoArrendatario.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openPhotoModalSafe('arrendatario');
        });
            ...
            if (savedData) {
                ...
                Object.keys(data).forEach(field => {
                    if (field === 'firmasDigitales') {
                        systemState.firmasDigitales = data.firmasDigitales || { propietario: false, arrendatario: false };
    ...
    clearForm() {
        this.showConfirmDialog(
            ...
            () => {
                ...
                systemState.firmasDigitales = { propietario: false, arrendatario: false };
    ...
    async generatePDF() {
        ...
        try {
            ...
            setTimeout(() => {
                doc.save(fileName);
                this.showNotification('PDF de Salvoconducto generado exitosamente', 'success');
            }, 1500);
    ...
    async addPDFContent(doc) {
        ...
        doc.setFontSize(14);
        doc.text('SALVOCONDUCTO ARRENDATARIO', 105, y, { align: 'center' });
        y += 30;
        ...
        const fechaLugar = `${ciudadDocumento}, ${fechaTexto}.`;
        doc.text(fechaLugar, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, fechaLugar, pageWidth - 2 * margin) + 20;
        ...
        const ciudadInmueble = document.getElementById('ciudadInmueble').value;

        const parrafo1 = `Yo, ${nombrePropietario}, cédula nacional de identidad número RUT ${rutPropietario}, propietario/a del inmueble ubicado en ${direccionInmueble}, comuna ${comunaInmueble}, ciudad ${ciudadInmueble}, país Chile.`;
        ...
        const parrafo2 = `Declaro que el arrendatario u ocupante don/doña ${nombreArrendatario}, cédula nacional de identidad número RUT ${rutArrendatario}, se encuentra al día en todos los pagos involucrados a este arrendamiento, incluyendo la renta correspondiente al último mes y los servicios con que cuenta el inmueble.`;
        doc.text(parrafo2, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, parrafo2, pageWidth - 2 * margin) + 20;

        // Declaración principal
        const parrafo3 = 'Por lo tanto, no hay impedimento legal, judicial ni contractual alguno de mi parte para que traslade sus bienes muebles del domicilio recién mencionado.';
        doc.text(parrafo3, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, parrafo3, pageWidth - 2 * margin) + 20;

        // Referencia legal
        const parrafo4 = 'Emito la presente declaración en cumplimiento con la Ley 20.227 a fin de que se obtenga la correspondiente declaración jurada.';
        doc.text(parrafo4, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, parrafo4, pageWidth - 2 * margin) + 20;
    ...
    addSignatureSection(doc, y) {
        ...
        const leftX = 60;   // Propietario
        const rightX = 150; // Arrendatario
        ...
        const nombrePropietario = document.getElementById('nombrePropietario').value;
        const nombreArrendatario = document.getElementById('nombreArrendatario').value;
        doc.text(nombrePropietario, leftX, y, { align: 'center' });
        doc.text(nombreArrendatario, rightX, y, { align: 'center' });
        y += 8;
        ...
        doc.setFont('times', 'normal');
        doc.setFontSize(9);
        doc.text(`RUT ${rutPropietario}`, leftX, y, { align: 'center' });
        doc.text(`RUT ${rutArrendatario}`, rightX, y, { align: 'center' });
        
        // Agregar firmas digitales si existen
        if (window.firmaProcessor) {
            try {
                const firmaPropietario = window.firmaProcessor.getSignatureForPDF('propietario');
                const firmaArrendatario = window.firmaProcessor.getSignatureForPDF('arrendatario');
                
                if (firmaPropietario) {
                    doc.addImage(firmaPropietario, 'PNG', leftX - 25, y - 30, 50, 20);
                    console.log('✅ Firma del propietario agregada al PDF');
                }
                
                if (firmaArrendatario) {
                    doc.addImage(firmaArrendatario, 'PNG', rightX - 25, y - 30, 50, 20);
                    console.log('✅ Firma del arrendatario agregada al PDF');
                }
                
                if (firmaPropietario || firmaArrendatario) {
                    doc.setFont('times', 'italic');
                    doc.setFontSize(8);
    ...
    generateFileName() {
        ...
        return `Salvoconducto_Mudanza_${nombrePropietario}_${timestamp}.pdf`;
    }
    ...
    validateFormWithSignatures() {
        ...
        const rutPropietario = document.getElementById('rutPropietario').value;
        if (rutPropietario && !ConfigUtils.validateRUT(rutPropietario)) {
            issues.push('RUT del propietario inválido');
        }
        ...
        if (!systemState.firmasDigitales.propietario) {
            issues.push('Falta firma digital del propietario');
        }
...
class FirmaDigitalProcessor {
    ...
    applySignature() {
        if (this.currentType && this.processedImageData) {
            ...
            if (window.salvoconductoSystem) {
                window.salvoconductoSystem.updateProgress();
            }
            
            this.closeModal();
            
            if (window.salvoconductoSystem) {
                window.salvoconductoSystem.showNotification(`Firma digital del ${this.currentType} aplicada correctamente`, 'success');
            }
        }
    }
    ...
    clearSignatures() {
        this.firmasDigitales = { propietario: null, arrendatario: null };
        
        systemState.firmasDigitales = { propietario: false, arrendatario: false };
        
        ['propietario', 'arrendatario'].forEach(type => {
...
class PhotoCarnetDualProcessor {
    ...
    handleCameraError(error) {
        ...
        if (window.salvoconductoSystem) {
            window.salvoconductoSystem.showNotification(errorMessage, 'error');
        }
    }
    ...
    capturePhoto() {
        ...
        try {
            ...
            if (window.salvoconductoSystem) {
                window.salvoconductoSystem.showNotification(`Foto del ${this.currentStep} capturada`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al capturar foto:', error);
            if (window.salvoconductoSystem) {
                window.salvoconductoSystem.showNotification('Error al capturar la foto', 'error');
            }
        }
    ...
    nextStep() {
        if (this.currentStep === 'frente' && this.tempImages.frente) {
            ...
            this.showCamera();
            if (window.salvoconductoSystem) {
                window.salvoconductoSystem.showNotification('Ahora capture el reverso del carnet', 'info');
            }
        } else if (this.currentStep === 'reverso' && this.tempImages.reverso) {
            this.currentStep = 'completed';
            this.showSummary();
            if (window.salvoconductoSystem) {
                window.salvoconductoSystem.showNotification('Ambas fotos capturadas', 'success');
            }
        }
    }
    ...
    restartProcess() {
        ...
        this.showCamera();
        if (window.salvoconductoSystem) {
            window.salvoconductoSystem.showNotification('Proceso reiniciado', 'info');
        }
    }
    ...
    async confirmAllPhotos() {
        if (!this.tempImages.frente || !this.tempImages.reverso || !this.currentType) {
            if (window.salvoconductoSystem) {
                window.salvoconductoSystem.showNotification('Faltan fotos por capturar', 'error');
            }
            return;
        }
        ...
        try {
            ...
            if (window.salvoconductoSystem) {
                window.salvoconductoSystem.showNotification(`Carnet completo del ${this.currentType} guardado`, 'success');
            }

        } catch (error) {
            console.error('❌ Error al confirmar fotos:', error);
            if (window.salvoconductoSystem) {
                window.salvoconductoSystem.showNotification('Error al procesar las fotos', 'error');
            }
        }
    }
...
window.clearForm = function() {
    if (window.salvoconductoSystem) {
        window.salvoconductoSystem.clearForm();
    }
};

window.generatePDF = function() {
    if (window.salvoconductoSystem) {
        window.salvoconductoSystem.generatePDF();
    }
};

// ===== INICIALIZACIÓN DEL SISTEMA =====
let salvoconductoSystem;

function initializeSalvoconductoSystem() {
    console.log('📋 Iniciando Sistema de Salvoconducto de Mudanza...');
    
    // Verificar que jsPDF esté disponible
    if (typeof window.jspdf === 'undefined') {
        console.warn('⚠️ jsPDF no está disponible, esperando...');
        setTimeout(initializeSalvoconductoSystem, 1000);
        return;
    }
    ...
    try {
        salvoconductoSystem = new SalvoconductoSystem();
        window.salvoconductoSystem = salvoconductoSystem;
        
        console.log('✅ Sistema completamente inicializado');
    } catch (error) {
        console.error('❌ Error crítico en inicialización:', error);
        setTimeout(initializeSalvoconductoSystem, 2000);
    }
}

// ===== MÚLTIPLES PUNTOS DE INICIALIZACIÓN =====
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeSalvoconductoSystem);
} else if (document.readyState === 'interactive' || document.readyState === 'complete') {
    setTimeout(initializeSalvoconductoSystem, 500);
}
...
window.addEventListener('error', (event) => {
    ...
    if (window.salvoconductoSystem) {
        window.salvoconductoSystem.showNotification(
...
// ===== LOG FINAL =====
console.log('📜 Sistema de Salvoconducto de Mudanza - Script principal cargado correctamente');