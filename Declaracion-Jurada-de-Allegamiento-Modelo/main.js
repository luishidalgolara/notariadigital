// ========================================
// 📋 SISTEMA PRINCIPAL DE DECLARACIÓN JURADA DE ALLEGAMIENTO
// ========================================
...
const systemState = {
    formFields: [
        ...
        'nombreDueno', 'rutDueno', 'nacionalidadDueno', 'estadoCivilDueno', 'profesionDueno', 'telefonoDueno', 'correoDueno',
        'direccionVivienda', 'comunaVivienda', 'regionVivienda', 'tipoVivienda', 'viviendaPropia',
        'fechaInicioAllegamiento', 'nombreAllegadoPrincipal', 'rutAllegadoPrincipal', 'relacionEstudiante',
        'nombresAllegados', 'numeroAllegados', 'menoresEdad',
        'motivoAllegamiento', 'descripcionMotivo', 'observacionesAdicionales', 'declaracionVerdadera'
    ],
...
class DeclaracionJuradaSystem {
    ...
    async init() {
        console.log('📋 Inicializando Sistema de Declaración Jurada de Allegamiento...');
    ...
    async ensureCriticalElements() {
        const criticalElements = [
            ...
            'firmaDueno', 'firmaAllegado', 'photoDueno', 'photoAllegado',
    ...
    setupSpecialFieldListeners() {
        // Validación RUT Dueño
        const rutDueno = document.getElementById('rutDueno');
        if (rutDueno) {
            rutDueno.addEventListener('input', (e) => {
                this.handleRutInput(e, 'rutDueno');
            });
        }

        // Validación RUT Allegado Principal
        const rutAllegadoPrincipal = document.getElementById('rutAllegadoPrincipal');
        if (rutAllegadoPrincipal) {
            rutAllegadoPrincipal.addEventListener('input', (e) => {
                this.handleRutInput(e, 'rutAllegadoPrincipal');
            });
        }

        // Validación Email Dueño
        const correoDueno = document.getElementById('correoDueno');
        if (correoDueno) {
            correoDueno.addEventListener('input', (e) => {
        ...
        if (telefonoDueno) {
            telefonoDueno.addEventListener('input', (e) => {
                if (ConfigUtils.validatePhone(e.target.value)) {
                    e.target.classList.add('email-valid');
                    e.target.classList.remove('email-invalid');
                } else if (e.target.value !== '') {
                    e.target.classList.add('email-invalid');
        ...
        // Mostrar/ocultar descripción del motivo
        const motivoAllegamiento = document.getElementById('motivoAllegamiento');
        if (motivoAllegamiento) {
            motivoAllegamiento.addEventListener('change', () => {
                this.updateMotivoDescription();
            });
        }
    ...
    setupDefaultValues() {
        ...
        const nacionalidadDueno = document.getElementById('nacionalidadDueno');
        if (nacionalidadDueno && !nacionalidadDueno.value) {
            nacionalidadDueno.value = 'Chilena';
        }

        // Valor por defecto para región
        const regionVivienda = document.getElementById('regionVivienda');
        if (regionVivienda && !regionVivienda.value) {
            regionVivienda.value = 'Región Metropolitana';
        }
    }
    ...
    updateDuplicateReferences() {
        ...
        const rutDueno = document.getElementById('rutDueno').value;
        const rutAllegadoPrincipal = document.getElementById('rutAllegadoPrincipal').value;

        // Actualizar referencias múltiples del RUT del dueño
        const prev_rutDueno2 = document.getElementById('prev_rutDueno2');
        if (prev_rutDueno2) prev_rutDueno2.textContent = rutDueno || 'XX.XXX.XXX-X';

        // Actualizar referencias múltiples del RUT del allegado
        const prev_rutAllegadoPrincipal2 = document.getElementById('prev_rutAllegadoPrincipal2');
        if (prev_rutAllegadoPrincipal2) prev_rutAllegadoPrincipal2.textContent = rutAllegadoPrincipal || 'XX.XXX.XXX-X';
    }
    ...
    updateDatePreview() {
        ...
        if (fechaDeclaracion && ConfigUtils.validateDate(fechaDeclaracion)) {
            ...
            if (prev_anio) prev_anio.textContent = '202_';
        }
    }

    updateMotivoDescription() {
        const motivoAllegamiento = document.getElementById('motivoAllegamiento').value;
        const descripcionSection = document.getElementById('descripcionMotivoSection');
        
        if (descripcionSection) {
            if (motivoAllegamiento === 'otro' || (motivoAllegamiento && document.getElementById('descripcionMotivo').value.trim())) {
    ...
    updateSignatureNames() {
        const nombreDueno = document.getElementById('nombreDueno').value;
        const nombreAllegadoPrincipal = document.getElementById('nombreAllegadoPrincipal').value;
        
        const firmaDueno = document.getElementById('prev_firmaDueno');
        const firmaAllegado = document.getElementById('prev_firmaAllegado');
        
        if (firmaDueno) {
            firmaDueno.textContent = nombreDueno || '_______________';
        }
        
        if (firmaAllegado) {
            firmaAllegado.textContent = nombreAllegadoPrincipal || '_______________';
        }
    }

    getPlaceholderText(field) {
        const placeholders = {
            'ciudadDeclaracion': 'Santiago',
            'nombreDueno': '________________',
            'rutDueno': 'XX.XXX.XXX-X',
            'nacionalidadDueno': 'Chilena',
            'estadoCivilDueno': '________',
            'profesionDueno': '________',
            'telefonoDueno': '________',
            'correoDueno': '________',
            'direccionVivienda': '________________',
            'comunaVivienda': '________',
            'regionVivienda': '________',
            'tipoVivienda': '________',
            'viviendaPropia': '________',
            'nombreAllegadoPrincipal': '________________',
            'rutAllegadoPrincipal': 'XX.XXX.XXX-X',
            'relacionEstudiante': '________',
            'nombresAllegados': '________________',
            'numeroAllegados': '__',
            'menoresEdad': '________',
            'motivoAllegamiento': '________',
            'descripcionMotivo': '________',
            'observacionesAdicionales': '________',
            'fechaInicioAllegamiento': '________________'
        };
    ...
    updateProgress() {
        ...
        if (systemState.firmasDigitales.dueno) bonusScore += 0.5;
        if (systemState.firmasDigitales.allegado) bonusScore += 0.5;
        
        // Bonus por fotos carnet
        if (systemState.fotosCarnet.dueno.completed) bonusScore += 0.5;
        if (systemState.fotosCarnet.allegado.completed) bonusScore += 0.5;
    ...
    async initializeSignatureSystemSafe() {
        ...
        const requiredElements = ['firmaModalOverlay', 'firmaDueno', 'firmaAllegado'];
    ...
    async setupSignatureButtonsSafe() {
        ...
        const btnFirmaDueno = document.getElementById('firmaDueno');
        const btnFirmaAllegado = document.getElementById('firmaAllegado');
        
        if (!btnFirmaDueno || !btnFirmaAllegado) {
            throw new Error('Botones de firma no encontrados');
        }
        
        btnFirmaDueno.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openSignatureModalSafe('dueno');
        });
        
        btnFirmaAllegado.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openSignatureModalSafe('allegado');
        });
    ...
    openSignatureModalSafe(type) {
        ...
        let nombreField;
        if (type === 'dueno') {
            nombreField = document.getElementById('nombreDueno');
        } else {
            nombreField = document.getElementById('nombreAllegadoPrincipal');
        }
        
        if (!nombreField || !nombreField.value.trim()) {
            this.showNotification(`Complete el nombre del ${type === 'dueno' ? 'dueño de la vivienda' : 'allegado principal'} antes de firmar`, 'warning');
            return;
        }
    ...
    async initializePhotoSystemSafe() {
        ...
        const requiredElements = ['photoModalOverlay', 'photoDueno', 'photoAllegado'];
        const missingElements = requiredElements.filter(id => !document.getElementById(id));
    ...
    async setupPhotoButtonsSafe() {
        ...
        const btnPhotoDueno = document.getElementById('photoDueno');
        const btnPhotoAllegado = document.getElementById('photoAllegado');
        
        if (!btnPhotoDueno || !btnPhotoAllegado) {
            throw new Error('Botones de foto no encontrados');
        }
        
        btnPhotoDueno.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openPhotoModalSafe('dueno');
        });
        
        btnPhotoAllegado.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.openPhotoModalSafe('allegado');
        });
    ...
    openPhotoModalSafe(type) {
        ...
        if (!nombreField || !nombreField.value.trim()) {
            this.showNotification(`Complete el nombre del ${type === 'dueno' ? 'dueño de la vivienda' : 'allegado principal'} antes de tomar fotos`, 'warning');
            return;
        }
    ...
    saveFormState() {
        ...
        try {
            localStorage.setItem('declaracion_jurada_form_data', JSON.stringify(data));
        } catch (error) {
    ...
    restoreFormState() {
        try {
            const savedData = localStorage.getItem('declaracion_jurada_form_data');
            if (savedData) {
                const data = JSON.parse(savedData);
                
                Object.keys(data).forEach(field => {
                    if (field === 'firmasDigitales') {
                        systemState.firmasDigitales = data.firmasDigitales || { dueno: false, allegado: false };
    ...
    clearForm() {
        this.showConfirmDialog(
            ...
            () => {
                ...
                systemState.firmasDigitales = { dueno: false, allegado: false };
                systemState.fotosCarnet = {
                    dueno: { frente: { captured: false, imageData: null, timestamp: null }, reverso: { captured: false, imageData: null, timestamp: null }, completed: false },
                    allegado: { frente: { captured: false, imageData: null, timestamp: null }, reverso: { captured: false, imageData: null, timestamp: null }, completed: false }
                };
    ...
    async addPDFContent(doc) {
        ...
        doc.setFontSize(16);
        doc.text('DECLARACIÓN JURADA', 105, y, { align: 'center' });
        y += 15;
        doc.setFontSize(18);
        doc.text('DE ALLEGAMIENTO', 105, y, { align: 'center' });
        y += 30;
        ...
        let fechaTexto = '__ de ________ del 202_';
        if (fechaDeclaracion && ConfigUtils.validateDate(fechaDeclaracion)) {
            const date = new Date(fechaDeclaracion);
            fechaTexto = `${date.getDate()} de ${this.mesesEspanol[date.getMonth()]} del ${date.getFullYear()}`;
        }

        const nombreDueno = document.getElementById('nombreDueno').value || '________________';
        const rutDueno = document.getElementById('rutDueno').value || 'XX.XXX.XXX-X';
        const direccionVivienda = document.getElementById('direccionVivienda').value || '________________';
        const comunaVivienda = document.getElementById('comunaVivienda').value || '________';
        const regionVivienda = document.getElementById('regionVivienda').value || '________';

        const parrafo1 = `Yo ${nombreDueno}, RUT ${rutDueno}, con fecha ${fechaTexto}, declaro bajo juramento que en mi casa ubicada en:`;
        ...
        doc.setFont('times', 'bold');
        doc.setFontSize(14);
        const direccionCompleta = `${direccionVivienda}, comuna de ${comunaVivienda}, ${regionVivienda}`;
        doc.text(direccionCompleta, 105, y, { align: 'center' });
        y += 20;

        doc.setFont('times', 'normal');
        doc.setFontSize(12);
        doc.text('Vive(n) en calidad de allegado(s):', margin, y);
        y += 15;

        // Información de los allegados
        const nombreAllegadoPrincipal = document.getElementById('nombreAllegadoPrincipal').value || '________________';
        const rutAllegadoPrincipal = document.getElementById('rutAllegadoPrincipal').value || 'XX.XXX.XXX-X';
        const relacionEstudiante = document.getElementById('relacionEstudiante').value || '________';
        const nombresAllegados = document.getElementById('nombresAllegados').value || '________________';
        const numeroAllegados = document.getElementById('numeroAllegados').value || '__';

        doc.setFont('times', 'bold');
        doc.text(`Allegado Principal: ${nombreAllegadoPrincipal}, RUT: ${rutAllegadoPrincipal}`, margin, y);
        y += 8;
        doc.setFont('times', 'normal');
        doc.text(`Relación con el estudiante: ${relacionEstudiante}`, margin, y);
        y += 15;

        doc.text('Todas las personas allegadas:', margin, y);
        y += 8;
        doc.text(nombresAllegados, margin + 5, y, {maxWidth: pageWidth - 2 * margin - 5});
        y += this.calculateTextHeight(doc, nombresAllegados, pageWidth - 2 * margin - 5) + 10;

        doc.text(`Número total de allegados: ${numeroAllegados} persona(s)`, margin, y);
        y += 20;
        ...
        // Fecha de inicio del allegamiento
        const fechaInicioAllegamiento = document.getElementById('fechaInicioAllegamiento').value;
        let fechaInicioTexto = '________________';
        if (fechaInicioAllegamiento && ConfigUtils.validateDate(fechaInicioAllegamiento)) {
            const date = new Date(fechaInicioAllegamiento);
            fechaInicioTexto = `${date.getDate()} de ${this.mesesEspanol[date.getMonth()]} del ${date.getFullYear()}`;
        }

        const parrafoFecha = `Desde ${fechaInicioTexto} (indicar fecha desde que el grupo familiar se encuentra como allegado).`;
        doc.text(parrafoFecha, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, parrafoFecha, pageWidth - 2 * margin) + 20;

        // Motivo del allegamiento
        const motivoAllegamiento = document.getElementById('motivoAllegamiento').value || '________';
        const descripcionMotivo = document.getElementById('descripcionMotivo').value;
        
        doc.setFont('times', 'bold');
        doc.text(`Motivo del allegamiento: ${motivoAllegamiento}`, margin, y);
        doc.setFont('times', 'normal');
        y += 10;

        if (descripcionMotivo && descripcionMotivo.trim()) {
            doc.text(`Descripción: ${descripcionMotivo}`, margin, y, {maxWidth: pageWidth - 2 * margin});
            y += this.calculateTextHeight(doc, descripcionMotivo, pageWidth - 2 * margin) + 15;
        } else {
            y += 10;
        }
        ...
        doc.setFontSize(10);
        const nota = 'Nota: Se entiende por allegado al grupo familiar que comparte una vivienda que no es propia y es habitada con otras personas o familia.';
        doc.text(nota, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, nota, pageWidth - 2 * margin) + 20;

        // Declaración de responsabilidad
        doc.setFont('times', 'normal');
        doc.setFontSize(12);
        const declaracion = 'Me hago responsable de la información entregada en este formulario y declaro bajo juramento que es fidedigna, autorizando a la institución solicitante a comprobar y requerir de quien corresponda los antecedentes mencionados.';
        doc.text(declaracion, margin, y, {maxWidth: pageWidth - 2 * margin});
        y += this.calculateTextHeight(doc, declaracion, pageWidth - 2 * margin) + 20;
    ...
    addSignatureSection(doc, y) {
        ...
        const nombreDueno = document.getElementById('nombreDueno').value || '_______________';
        const nombreAllegadoPrincipal = document.getElementById('nombreAllegadoPrincipal').value || '_______________';
        const rutDueno = document.getElementById('rutDueno').value || 'XX.XXX.XXX-X';
        const rutAllegadoPrincipal = document.getElementById('rutAllegadoPrincipal').value || 'XX.XXX.XXX-X';
        ...
        doc.setFont('times', 'bold');
        doc.setFontSize(11);
        doc.text(nombreDueno, leftX, y, { align: 'center' });
        doc.text(nombreAllegadoPrincipal, rightX, y, { align: 'center' });
        y += 6;
        
        // Roles
        doc.setFont('times', 'normal');
        doc.setFontSize(10);
        doc.text('DUEÑO DE LA VIVIENDA', leftX, y, { align: 'center' });
        doc.text('ALLEGADO PRINCIPAL', rightX, y, { align: 'center' });
        y += 6;

        // RUTs
        doc.setFontSize(9);
        doc.text(`RUT: ${rutDueno}`, leftX, y, { align: 'center' });
        doc.text(`RUT: ${rutAllegadoPrincipal}`, rightX, y, { align: 'center' });
        
        // Agregar firmas digitales si existen
        if (window.firmaProcessor) {
            try {
                const firmaDueno = window.firmaProcessor.getSignatureForPDF('dueno');
                const firmaAllegado = window.firmaProcessor.getSignatureForPDF('allegado');
                
                if (firmaDueno) {
                    doc.addImage(firmaDueno, 'PNG', leftX - 30, y - 25, 60, 15);
                    console.log('✅ Firma del dueño agregada al PDF');
                }
                
                if (firmaAllegado) {
                    doc.addImage(firmaAllegado, 'PNG', rightX - 30, y - 25, 60, 15);
                    console.log('✅ Firma del allegado agregada al PDF');
                }
                
                if (firmaDueno || firmaAllegado) {
                    doc.setFont('times', 'italic');
                    doc.setFontSize(8);
    ...
    generateFileName() {
        const nombreDueno = document.getElementById('nombreDueno').value
            .replace(/\s+/g, '_')
            .replace(/[^\w\-]/g, '');
        const nombreAllegado = document.getElementById('nombreAllegadoPrincipal').value
            .replace(/\s+/g, '_')
            .replace(/[^\w\-]/g, '');
        const timestamp = new Date().toISOString().slice(0, 10);
        return `Declaracion_Jurada_Allegamiento_${nombreDueno}_${nombreAllegado}_${timestamp}.pdf`;
    }
    ...
    validateFormWithSignatures() {
        ...
        const requiredFields = [
            'ciudadDeclaracion', 'fechaDeclaracion',
            'nombreDueno', 'rutDueno', 
            'direccionVivienda', 'comunaVivienda', 'regionVivienda',
        ...
        const declaracionVerdadera = document.getElementById('declaracionVerdadera');
        if (!declaracionVerdadera || !declaracionVerdadera.checked) {
            emptyFields.push('declaracionVerdadera');
        }
        ...
        if (rutDueno && !ConfigUtils.validateRUT(rutDueno)) {
            issues.push('RUT del dueño inválido');
        }
        
        if (rutAllegadoPrincipal && !ConfigUtils.validateRUT(rutAllegadoPrincipal)) {
            issues.push('RUT del allegado principal inválido');
        }
        
        // Validar fechas
        const fechaDeclaracion = document.getElementById('fechaDeclaracion').value;
        if (fechaDeclaracion && !ConfigUtils.validateDate(fechaDeclaracion)) {
            issues.push('Fecha de la declaración inválida');
        }

        const fechaInicioAllegamiento = document.getElementById('fechaInicioAllegamiento').value;
        if (fechaInicioAllegamiento && !ConfigUtils.validateDate(fechaInicioAllegamiento)) {
            issues.push('Fecha de inicio del allegamiento inválida');
        }
        
        if (!systemState.firmasDigitales.dueno) {
            issues.push('Falta firma digital del dueño');
        }
        
        if (!systemState.firmasDigitales.allegado) {
            issues.push('Falta firma digital del allegado');
        }
...
class FirmaDigitalProcessor {
    ...
    applySignature() {
        if (this.currentType && this.processedImageData) {
            ...
            if (window.declaracionJuradaSystem) {
                window.declaracionJuradaSystem.showNotification(`Firma digital del ${this.currentType === 'dueno' ? 'dueño' : 'allegado'} aplicada correctamente`, 'success');
            }
        }
    }
    ...
    clearSignatures() {
        this.firmasDigitales.dueno = null;
        this.firmasDigitales.allegado = null;
        
        systemState.firmasDigitales = { dueno: false, allegado: false };
        
        ['firmaDueno', 'firmaAllegado'].forEach(buttonId => {
            const button = document.getElementById(buttonId);
            if (button) {
        ...
        ['signaturePlaceholderDueno', 'signaturePreviewDueno', 'signaturePlaceholderAllegado', 'signaturePreviewAllegado'].forEach((id, index) => {
...
class PhotoCarnetDualProcessor {
    ...
    async confirmAllPhotos() {
        ...
        try {
            ...
            if (window.declaracionJuradaSystem) {
                window.declaracionJuradaSystem.showNotification(`Carnet completo del ${this.currentType === 'dueno' ? 'dueño' : 'allegado'} guardado`, 'success');
            }
